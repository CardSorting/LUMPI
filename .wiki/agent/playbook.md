...
- Codemarie Bridge Engine Fusion Architecture: `packages/codemarie/src/index.ts` & `packages/coding-agent/src/core/codemarie-bridge.ts`. Exposes 11 unified engine subsystems: `WorkspaceIntelligenceEngine`, `SpiderEngine`, `PersistentSubscriptionHub`, `CommandExecutor`, `ContextStalenessTracker`, `commandSanitizer` (`validateCommand`, `splitCommand`, `getSanitizerMode`), `DietCodeTempManager`, `SwarmMutexService`, `RoadmapGateCatalog` (`buildGateStateFromInputs`, `collectGateInputs`, `evaluateGateChecks`), `TelemetryService` / `telemetryService`, document text extraction (`extractTextFromFile`, `callTextExtractionFunctions`, `processFilesIntoText`, `sanitizeNotebookForLLM`), code safety omission detector (`detectCodeOmission`, `showOmissionWarning`), `CommentReviewController`, `content-limits` (`MAX_CONTENT_SIZE_BYTES`, `formatBytes`, `truncateContent`), `BroccoliFencingAdapter` (`readBroccoliFence`, `broccoliFencePath`), and `RoadmapAutoGovernance` (`AUTO_GOVERNANCE`, `ROADMAP_DIAGNOSTIC_SLASH_COMMANDS`, `governanceFieldsFromStatus`).
