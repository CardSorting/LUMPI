...
| `Cannot find name 'Bun'` or `Cannot find module 'bun'` under `--moduleResolution node16` | Missing ambient Bun types or hard dependency on `Bun.file`/`Bun.write` in Node ESM builds | Inspect `git.ts` or `tsconfig.json` | Create `node_modules/@types/bun/index.d.ts` and use standard `node:fs/promises` (`writeFile`, `readFile`) | `bun packages/coding-agent/src/cli.ts --smoke-test` and `npm run check` pass |
| Bun worker thread drops parent process messages during async top-level import | Event listeners attached after top-level `await import(...)` in worker thread miss initial message flush | Inspect worker spawn entry point | Register `installWorkerInbox()` synchronously at worker top-level startup; replay via `consumeWorkerInbox()` | `bun packages/coding-agent/src/cli.ts --smoke-test` reports `pi native worker smoke test passed` |
| AI provider payload rejected due to un-clamped thinking effort level | Provider requires specific effort string (e.g., Anthropic adaptive, Gemini 3, OpenAI o-series) | Check API request parameters in `packages/catalog` | Use `resolveModelThinking()` clampers from `@oh-my-pi/pi-catalog` | `npm run check` and model resolution benchmarks pass |
