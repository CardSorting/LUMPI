...
import {
	...
	parseFileHunks,
	parseNumstat,
} from "../commit/git/diff.ts";
import type { FileDiff, FileHunks, NumstatEntry } from "../commit/types.ts";
import { ToolAbortError, ToolError, throwIfAborted } from "../tools/tool-errors.ts";
...
async function waitForChildExit(child: Subprocess, timeoutMs: number): Promise<boolean> {
	...
	const timeout = createDeferred<false>();
...
async function waitForExitWithTimeout(
	...
	const timeout = createDeferred<"timeout">();
	const timer = setTimeout(() => timeout.resolve("timeout"), timeoutMs);
	timer.unref?.();
	try {
		const result = await Promise.race([
			child.exited.then((exitCode: number | null) => ({ kind: "exit" as const, exitCode })),
...
function gitSpawnSyncText(
	...
	try {
		const bunInstance = (globalThis as unknown as { Bun?: any }).Bun;
		const result = bunInstance.spawnSync(["git", ...commandArgs], {
			cwd,
			env: buildGitEnv(),
			stdout: "pipe",
			stderr: "pipe",
...
async function git(cwd: string, args: readonly string[], options: CommandOptions = {}): Promise<GitCommandResult> {
	...
	try {
		const bunInstance = (globalThis as unknown as { Bun?: any }).Bun;
		child = bunInstance.spawn(["git", ...commandArgs], {
			cwd,
...
async function readOptionalBytes(filePath: string): Promise<Uint8Array | null> {
	return retryOnEintr(async () => new Uint8Array(await fs.promises.readFile(filePath)));
}
...
export async function detachGitDir(worktreeRoot: string, sourceCommonDir: string): Promise<DetachGitDirResult> {
	...
	if (headSha) {
		await fs.promises.writeFile(path.join(gitEntry, "HEAD"), `${headSha}\n`);
		if (refDump) {
		...
		if (headRef) await fs.promises.writeFile(path.join(gitEntry, "HEAD"), `ref: ${headRef}\n`);
	} else if (headRef && !initialBranch) {
		// Unborn detached HEAD (no branch, no commit) — restore the raw ref target.
		await fs.promises.writeFile(path.join(gitEntry, "HEAD"), `ref: ${headRef}\n`);
	}
	...
	if (shallowBoundary !== null) await fs.promises.writeFile(path.join(gitEntry, "shallow"), shallowBoundary);
	...
	if (sparsePatterns !== null) {
		...
		await fs.promises.writeFile(path.join(infoDir, "sparse-checkout"), sparsePatterns);
	}
	...
	if (indexBytes) {
		for (const shared of sharedIndexFiles) {
			await fs.promises.writeFile(path.join(gitEntry, shared.name), shared.bytes);
		}
		await fs.promises.writeFile(path.join(gitEntry, "index"), indexBytes);
	} else if (headSha) {
...
export const github = {
	...
	async run(cwd: string, args: string[], signal?: AbortSignal, options?: GhCommandOptions): Promise<GhCommandResult> {
		...
		try {
			const bunInstance = (globalThis as unknown as { Bun?: any }).Bun;
			const child = bunInstance.spawn(["gh", ...args], {
				cwd,
				env: buildGhEnv(),
				stdin: "ignore",
				stdout: "pipe",
