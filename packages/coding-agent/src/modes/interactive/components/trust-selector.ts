...
export class TrustSelectorComponent extends Container {
	...
	constructor(options: TrustSelectorOptions) {
		...
		this.addChild(new Text(theme.fg("accent", theme.bold("LUMI Project Security & Trust Authority")), 1, 0));
	...
	handleInput(keyData: string): void {
		...
		if (kb.matches(keyData, "tui.select.up") || keyData === "k") {
			if (this.trustOptions.length === 0) return;
			this.selectedIndex = this.selectedIndex === 0 ? this.trustOptions.length - 1 : this.selectedIndex - 1;
			this.updateList();
		} else if (kb.matches(keyData, "tui.select.down") || keyData === "j") {
			if (this.trustOptions.length === 0) return;
			this.selectedIndex = this.selectedIndex === this.trustOptions.length - 1 ? 0 : this.selectedIndex + 1;
			this.updateList();
