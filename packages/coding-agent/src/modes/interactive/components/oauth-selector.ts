...
export class OAuthSelectorComponent extends Container implements Focusable {
	...
	constructor(
		...
		const title = mode === "login" ? "Select Authentication Provider" : "Provider Logout";
		const subtitle =
			mode === "login"
				? "Choose a provider to authenticate or add API credentials:"
				: "Select an active provider to disconnect:";
		this.addChild(new TruncatedText(theme.fg("accent", theme.bold(title)), 1, 0));
		this.addChild(new TruncatedText(theme.fg("muted", subtitle), 1, 0));
		this.addChild(new Spacer(1));
		...
		this.addChild(new TruncatedText(`  ${navHints}`, 1, 0));
