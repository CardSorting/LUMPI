...
import { getKeybindings, type Keybinding, type KeyId } from "@noorm/lumpi-tui";
...
export function keyHint(keybinding: Keybinding, description: string): string {
	...
	return theme.fg("accent", `[${text}]`) + theme.fg("muted", ` ${description}`);
}

export function rawKeyHint(key: string, description: string): string {
	const text = formatKeyText(key);
	return theme.fg("accent", `[${text}]`) + theme.fg("muted", ` ${description}`);
}
