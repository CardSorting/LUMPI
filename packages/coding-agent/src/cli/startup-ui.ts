import { ProcessTerminal, setKeybindings, type TUI, TuiMainScreen } from "@noorm/lumi-tui";
...
const OFFICIAL_PACKAGE_NAME = "@noorm/lumi-coding-agent";
		...
		const finish = async (result: FirstTimeSetupResult | undefined) => {
			...
			if (result) {
				...
				if (result.defaultProvider) {
					settingsManager.setDefaultProvider(result.defaultProvider);
				}
