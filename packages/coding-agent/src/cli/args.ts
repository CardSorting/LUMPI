...
export function parseArgs(args: string[]): Args {
	...
	for (let i = 0; i < args.length; i++) {
		...
		if (arg === "--help" || arg === "-h") {
			...
			result.engine = arg.slice("--engine=".length);
...
${chalk.bold("Commands:")}
  ...
  ${APP_NAME} update [source|self|lumi] Update ${APP_NAME}, extensions, or model catalogs
...
${chalk.bold("Options:")}
  --provider <name>              Provider name (default: openai-codex)
  ...
  --mod                          Enable Mixture-of-Designers (MoD) prompt steering
  --engine <name>                Select execution engine (choices: codemarie, standard)
