...
Pi bundles core packages for extensions and skills. If you import any of these, list them in `peerDependencies` with a `"*"` range and do not bundle them: `@noorm/lumpi-ai`, `@noorm/lumpi-agent-core`, `@noorm/lumpi-coding-agent`, `@noorm/lumpi-tui`, `typebox`.
