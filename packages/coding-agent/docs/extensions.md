...
| Package | Purpose |
|---------|---------|
| `@noorm/lumpi` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
| `typebox` | Schema definitions for tool parameters |
| `@noorm/lumpi-ai` | AI utilities (`StringEnum` for Google-compatible enums) |
| `@noorm/lumpi-tui` | TUI components for custom rendering |
...
```typescript
import { isToolCallEventType } from "@noorm/lumpi";
...
```typescript
import { isBashToolResult } from "@noorm/lumpi";
...
```typescript
import { createLocalBashOperations } from "@noorm/lumpi";
...
```typescript
import { CONFIG_DIR_NAME, type ExtensionAPI } from "@noorm/lumpi";
...
```typescript
import { SessionManager } from "@noorm/lumpi";
...
```typescript
import type { AutocompleteItem } from "@noorm/lumpi-tui";
...
Extensions that need native provider auth, filtering, refresh, or stream behavior can register a complete `Provider` from `@noorm/lumpi-ai`. The provider becomes the composition base and `models.json` overrides still apply above it.

```typescript
import { createProvider, openAICompletionsApi } from "@noorm/lumpi-ai";
...
```typescript
import { withFileMutationQueue } from "@noorm/lumpi";
...
**Important:** Use `StringEnum` from `@noorm/lumpi-ai` for string enums. `Type.Union`/`Type.Literal` doesn't work with Google's API.
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "@noorm/lumpi";
...
```typescript
import { createBashTool } from "@noorm/lumpi";
...
```typescript
import { keyHint } from "@noorm/lumpi";
...
```typescript
import { Text, Component } from "@noorm/lumpi-tui";
...
import { matchesKey } from "@noorm/lumpi-tui";
...
```typescript
import { highlightCode, getLanguageFromPath } from "@noorm/lumpi";
