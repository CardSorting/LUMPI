...
```typescript
import type { ExtensionAPI } from "@noorm/lumi";
...
| Package | Purpose |
|---------|---------|
| `@noorm/lumi` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
| `typebox` | Schema definitions for tool parameters |
| `@noorm/lumi-ai` | AI utilities (`StringEnum` for Google-compatible enums) |
| `@noorm/lumi-tui` | TUI components for custom rendering |
...
```typescript
import { isToolCallEventType } from "@noorm/lumi";
...
```typescript
import { isBashToolResult } from "@noorm/lumi";
...
```typescript
import { createLocalBashOperations } from "@noorm/lumi";
...
```typescript
import { CONFIG_DIR_NAME, type ExtensionAPI } from "@noorm/lumi";
...
```typescript
import { SessionManager } from "@noorm/lumi";
...
```typescript
import type { AutocompleteItem } from "@noorm/lumi-tui";
...
Extensions that need native provider auth, filtering, refresh, or stream behavior can register a complete `Provider` from `@noorm/lumi-ai`. The provider becomes the composition base and `models.json` overrides still apply above it.

```typescript
import { createProvider, openAICompletionsApi } from "@noorm/lumi-ai";
...
```typescript
import { withFileMutationQueue } from "@noorm/lumi";
...
**Important:** Use `StringEnum` from `@noorm/lumi-ai` for string enums. `Type.Union`/`Type.Literal` doesn't work with Google's API.
...
- [read.ts](https://github.com/earendil-works/lumpi/blob/main/packages/coding-agent/src/core/tools/read.ts) - `ReadToolDetails`
- [bash.ts](https://github.com/earendil-works/lumpi/blob/main/packages/coding-agent/src/core/tools/bash.ts) - `BashToolDetails`
- [edit.ts](https://github.com/earendil-works/lumpi/blob/main/packages/coding-agent/src/core/tools/edit.ts)
- [write.ts](https://github.com/earendil-works/lumpi/blob/main/packages/coding-agent/src/core/tools/write.ts)
- [grep.ts](https://github.com/earendil-works/lumpi/blob/main/packages/coding-agent/src/core/tools/grep.ts) - `GrepToolDetails`
- [find.ts](https://github.com/earendil-works/lumpi/blob/main/packages/coding-agent/src/core/tools/find.ts) - `FindToolDetails`
- [ls.ts](https://github.com/earendil-works/lumpi/blob/main/packages/coding-agent/src/core/tools/ls.ts) - `LsToolDetails`
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "@noorm/lumi";
...
```typescript
import { createBashTool } from "@noorm/lumi";
...
Tools can provide `renderCall` and `renderResult` for custom TUI display. See [tui.md](tui.md) for the full component API and [tool-execution.ts](https://github.com/earendil-works/lumpi/blob/main/packages/coding-agent/src/modes/interactive/components/tool-execution.ts) for how tool rows are composed.
...
```typescript
import { keyHint } from "@noorm/lumi";
...
```typescript
import { Text, Component } from "@noorm/lumi-tui";
...
```typescript
import { CustomEditor, type ExtensionAPI } from "@noorm/lumi";
import { matchesKey } from "@noorm/lumi-tui";
...
```typescript
import { highlightCode, getLanguageFromPath } from "@noorm/lumi";
