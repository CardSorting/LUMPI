import { stripWindowsExtendedLengthPathPrefix } from "./path.ts";
...
export function declareWorkerHostEntry(): void {
	const mainPath = (globalThis as unknown as { Bun?: { main: string } }).Bun?.main ?? process.argv[1] ?? "";
	workerHostMain = stripWindowsExtendedLengthPathPrefix(mainPath);
}
