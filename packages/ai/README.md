...
```bash
npm install @noorm/lumpi-ai
```

TypeBox exports are re-exported from `@noorm/lumpi-ai`: `Type`, `Static`, and `TSchema`.
...
```typescript
import { Type, type Context, type Tool } from '@noorm/lumpi-ai';
import { builtinModels } from '@noorm/lumpi-ai/providers/all';
...
import { openaiProvider } from '@noorm/lumpi-ai/providers/openai';
import { openrouterProvider } from '@noorm/lumpi-ai/providers/openrouter';
import { amazonBedrockProvider } from '@noorm/lumpi-ai/providers/amazon-bedrock';
...
```typescript
import { hasApi } from '@noorm/lumpi-ai';
...
```typescript
import { getBuiltinModel, getBuiltinModels, getBuiltinProviders } from '@noorm/lumpi-ai/providers/all';
...
```typescript
import { createModels, type CredentialStore } from '@noorm/lumpi-ai';
...
```typescript
import { Type, type Tool, StringEnum } from '@noorm/lumpi-ai';
...
```typescript
import { validateToolCall, type Tool } from '@noorm/lumpi-ai';
...
```typescript
import { builtinImagesModels } from '@noorm/lumpi-ai/providers/all';
...
```typescript
import { getImageModel, generateImages } from '@noorm/lumpi-ai/compat';
...
```typescript
import { createModels, createProvider, envApiKeyAuth, type Model } from '@noorm/lumpi-ai';
import { openAICompletionsApi } from '@noorm/lumpi-ai/api/openai-completions.lazy';
...
```typescript
import { anthropicMessagesApi } from '@noorm/lumpi-ai/api/anthropic-messages.lazy';
import { openAIResponsesApi } from '@noorm/lumpi-ai/api/openai-responses.lazy';
...
```typescript
import { stream } from '@noorm/lumpi-ai/api/anthropic-messages';
...
Importing an implementation module loads its SDK. The `./api/<id>.lazy` wrappers (used by the provider factories) defer that load to the first request when the runtime or bundler supports dynamic import chunking. Legacy raw API subpaths from older releases (`./anthropic`, `./google`, `./mistral`, `./openai-completions`, ...) were removed; use `@noorm/lumpi-ai/api/<api-id>`.
...
```typescript
import { createModels, type Context } from '@noorm/lumpi-ai';
import { anthropicProvider } from '@noorm/lumpi-ai/providers/anthropic';
import { openaiProvider } from '@noorm/lumpi-ai/providers/openai';
import { googleProvider } from '@noorm/lumpi-ai/providers/google';
...
- `@noorm/lumpi-ai` is the core entrypoint and does not import built-in catalogs, provider factories, or SDK implementations.
- `@noorm/lumpi-ai/providers/<provider>` imports that provider's catalog and lazy API wrapper only.
- `@noorm/lumpi-ai/providers/all` imports every built-in provider factory and all catalogs. Use it only when you want the full built-in set.
...
- Importing `@noorm/lumpi-ai/api/<api-id>` directly loads that API implementation and its SDK immediately.

Avoid `@noorm/lumpi-ai/compat` in new bundled apps; it preserves the old global API and imports the full built-in catalog surface.
...
```typescript
import { setBedrockProviderModule } from '@noorm/lumpi-ai/api/bedrock-converse-stream.lazy';
...
```bash
npx @noorm/lumpi-ai login              # interactive provider selection
npx @noorm/lumpi-ai login anthropic    # login to specific provider
npx @noorm/lumpi-ai list               # list available providers
```
...
Built-in login and refresh flows are private provider implementations. Use provider-owned `OAuthAuth`, which composes with `CredentialStore` and gets locked auto-refresh through `Models`. The `@noorm/lumpi-ai/oauth` entry point retains only type declarations required by coding-agent extension OAuth compatibility.
...
```typescript
// Before
import { getModel, complete } from '@noorm/lumpi-ai';

// After (verbatim behavior, one import-path change)
import { getModel, complete } from '@noorm/lumpi-ai/compat';
```
...
| `streamAnthropic(model, ctx, opts)` | `stream` from `@noorm/lumpi-ai/api/anthropic-messages`, or a provider in a collection |
...
Add a lazy wrapper `src/api/<api-id>.lazy.ts` (`<name>Api()` via `lazyApi()`) so providers can reference the implementation without importing its SDK. Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@noorm/lumpi-ai`.
