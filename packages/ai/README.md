...
```bash
npm install @noorm/lumi-ai
```

TypeBox exports are re-exported from `@noorm/lumi-ai`: `Type`, `Static`, and `TSchema`.
...
```typescript
import { Type, type Context, type Tool } from '@noorm/lumi-ai';
import { builtinModels } from '@noorm/lumi-ai/providers/all';
...
import { openaiProvider } from '@noorm/lumi-ai/providers/openai';
import { openrouterProvider } from '@noorm/lumi-ai/providers/openrouter';
import { amazonBedrockProvider } from '@noorm/lumi-ai/providers/amazon-bedrock';
...
```typescript
import { hasApi } from '@noorm/lumi-ai';
...
```typescript
import { getBuiltinModel, getBuiltinModels, getBuiltinProviders } from '@noorm/lumi-ai/providers/all';
...
```typescript
import { createModels, type CredentialStore } from '@noorm/lumi-ai';
...
```typescript
import { Type, type Tool, StringEnum } from '@noorm/lumi-ai';
...
```typescript
import { validateToolCall, type Tool } from '@noorm/lumi-ai';
...
```typescript
import { builtinImagesModels } from '@noorm/lumi-ai/providers/all';
...
```typescript
import { getImageModel, generateImages } from '@noorm/lumi-ai/compat';
...
```typescript
import { createModels, createProvider, envApiKeyAuth, type Model } from '@noorm/lumi-ai';
import { openAICompletionsApi } from '@noorm/lumi-ai/api/openai-completions.lazy';
...
```typescript
import { anthropicMessagesApi } from '@noorm/lumi-ai/api/anthropic-messages.lazy';
import { openAIResponsesApi } from '@noorm/lumi-ai/api/openai-responses.lazy';
...
```typescript
import { stream } from '@noorm/lumi-ai/api/anthropic-messages';
...
```typescript
import { createModels, type Context } from '@noorm/lumi-ai';
import { anthropicProvider } from '@noorm/lumi-ai/providers/anthropic';
import { openaiProvider } from '@noorm/lumi-ai/providers/openai';
import { googleProvider } from '@noorm/lumi-ai/providers/google';
...
- `@noorm/lumi-ai` is the core entrypoint and does not import built-in catalogs, provider factories, or SDK implementations.
- `@noorm/lumi-ai/providers/<provider>` imports that provider's catalog and lazy API wrapper only.
- `@noorm/lumi-ai/providers/all` imports every built-in provider factory and all catalogs. Use it only when you want the full built-in set.
...
- Importing `@noorm/lumi-ai/api/<api-id>` directly loads that API implementation and its SDK immediately.

Avoid `@noorm/lumi-ai/compat` in new bundled apps; it preserves the old global API and imports the full built-in catalog surface.
...
```typescript
import { setBedrockProviderModule } from '@noorm/lumi-ai/api/bedrock-converse-stream.lazy';
...
```bash
npx @noorm/lumi-ai login              # interactive provider selection
npx @noorm/lumi-ai login anthropic    # login to specific provider
npx @noorm/lumi-ai list               # list available providers
```
...
Built-in login and refresh flows are private provider implementations. Use provider-owned `OAuthAuth`, which composes with `CredentialStore` and gets locked auto-refresh through `Models`. The `@noorm/lumi-ai/oauth` entry point retains only type declarations required by coding-agent extension OAuth compatibility.
...
```typescript
// Before
import { getModel, complete } from '@noorm/lumi-ai';

// After (verbatim behavior, one import-path change)
import { getModel, complete } from '@noorm/lumi-ai/compat';
```
...
| `streamAnthropic(model, ctx, opts)` | `stream` from `@noorm/lumi-ai/api/anthropic-messages`, or a provider in a collection |
...
Add a lazy wrapper `src/api/<api-id>.lazy.ts` (`<name>Api()` via `lazyApi()`) so providers can reference the implementation without importing its SDK. Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@noorm/lumi-ai`.
