...
LUMI provides comprehensive support for Jupyter notebooks (`.ipynb` files), enabling AI-assisted editing with full cell-level context awareness. This feature was developed in collaboration with Amazon to bring AI coding assistance to data science workflows.

## Getting Started

Jupyter notebook support is built into LUMI. With the Jupyter extension enabled in VS Code, open any `.ipynb` file to use AI-assisted cell commands (`lumi.jupyterGenerateCell`, `lumi.jupyterExplainCell`, `lumi.jupyterImproveCell` in `package.json`).
...
LUMI extracts the full cell context, including outputs, so explanations can reference actual results like column names, row counts, and computed values.
...
LUMI suggests improvements while preserving the cell's position and metadata in the notebook structure. The AI explains what was changed and why.
...
When you use a Jupyter command, LUMI extracts structured context that includes:
...
LUMI is designed to work carefully with the cell JSON structure, aiming to:
...
- [All LUMI Tools](/tools-reference/all-dietcode-tools) - Overview of all LUMI tools
