...
When your conversation approaches the model's context window limit, LUMI automatically summarizes it to free up space and keep working.
...
LUMI monitors token usage during your conversation. When you're getting close to the limit, it:
...
Previously, LUMI would truncate older messages when hitting context limits, losing important context.
...
- LUMI remembers everything it's done
- You can work on much larger projects without interruption

<Tip>
Auto Compact works beautifully with [Focus Chain](/features/focus-chain). When Focus Chain is enabled, todo lists persist across summarizations. LUMI can work on long-horizon tasks spanning multiple context windows while staying on track.
</Tip>
...
<Note>
With other models, LUMI falls back to standard rule-based context truncation, even if Auto Compact is enabled.
</Note>
