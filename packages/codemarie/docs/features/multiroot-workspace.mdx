...
<Note>
While LUMI detects VCS information for all workspace folders, certain features only use the **primary workspace** (the first folder): [project rules](/customization/dietcode-rules), [workflows](/customization/workflows), and [Git-related features](/core-workflows/working-with-files) like `@git` mentions.
</Note>
...
[Project rules](/customization/dietcode-rules) (`.dietcoderules/` directory) only work in the **primary workspace** (the first folder in your workspace). Rules in other workspace folders are ignored.
...
- Add a `.dietcodeignore` file to reduce noise, speed up scanning, and keep LUMI focused on source code:
