...
description: "Find MCP servers, add them to LUMI, and configure settings."
---

This guide covers finding MCP servers, adding them to LUMI, and configuring their settings.

## Finding MCP Servers

Several places to find MCP servers:

- **[LUMI MCP Marketplace](/mcp/mcp-marketplace)**: Browse and install directly from LUMI
...
## Adding Servers with LUMI

The easiest way to add an MCP server is to have LUMI build it for you:

1. Provide LUMI with the GitHub repository URL
2. Optionally include the README contents for context
3. LUMI clones the repo, builds it, and adds the configuration

Example conversation:
```text
User: "Add the MCP server from https://github.com/modelcontextprotocol/servers/tree/main/src/brave-search"

LUMI: "Cloning the repository. It needs to be built. Should I run 'npm run build'?"

User: "Yes"

LUMI: "Build complete. This server needs an API key. Where should I find it?"
```

## Managing Servers

Access server settings through the MCP Servers icon in LUMI's top navigation bar.
...
4. Find `LUMI>Mcp:Mode` and select your preference
...
After configuring a server, LUMI automatically detects available tools:

1. Type your request in chat
2. LUMI identifies when an MCP tool can help
