...
description: "This protocol is designed to streamline the development process of building MCP servers with LUMI."
---

> **Build and share your MCP servers with the world.** Once you've created a great MCP server, submit it to the [LUMI MCP Marketplace](https://github.com/dietcode/mcp-marketplace) to make it discoverable and one-click installable by thousands of developers.

## What Are MCP Servers?

Model Context Protocol (MCP) servers extend AI assistants like LUMI by giving them the ability to:
...
The heart of effective MCP server development is following a structured protocol. This protocol is implemented through a `.dietcoderules` file that lives at the **root** of your MCP working directory (/Users/your-name/Documents/LUMI/MCP).

### Using `.dietcoderules` Files

A `.dietcoderules` file is a special configuration that LUMI reads automatically when working in the directory where it's placed. These files:

-   Configure LUMI's behavior and enforce best practices
-   Switch LUMI into a specialized MCP development mode
...
When this `.dietcoderules` file is present in your working directory, LUMI will:
...
First, add a `.dietcoderules` file to the root of your MCP working directory using the protocol above. This file configures LUMI to use the MCP development protocol when working in this folder.

### 2. Start a Chat with a Clear Description

Begin your LUMI chat by clearly describing what you want to build. Be specific about:
...
LUMI will automatically start in PLAN MODE, guiding you through the planning process:
...
When planning is complete, LUMI presents the plan in chat and **automatically** transitions to implementation — no manual mode switch required.

### 4. Provide API Documentation Early

One of the most effective ways to help LUMI build your MCP server is to share official API documentation right at the start:
...
Providing comprehensive API details (endpoints, authentication, data structures) significantly improves LUMI's ability to implement an effective MCP server.

## Understanding the Two Modes

### PLAN MODE

In this collaborative phase, you work with LUMI to design your MCP server:
...
Once planning is complete, LUMI helps implement the server:
...
Resources let your MCP servers expose data to LUMI without executing code. They're perfect for providing context like files, API responses, or database records that LUMI can reference during conversations.
...
Resources make your MCP servers more context-aware, allowing LUMI to access specific information without requiring you to copy/paste. For more information, refer to the [official documentation](https://modelcontextprotocol.io/docs/concepts/resources).
