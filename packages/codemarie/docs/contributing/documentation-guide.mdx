...
description: "How to write and contribute to LUMI documentation"
---

LUMI's documentation lives in the `docs/` directory and uses [Mintlify](https://mintlify.com) for rendering. This guide covers how to write docs that match LUMI's established style.

## Using the Documentation Workflow

The fastest way to create documentation is using the `/write-docs` workflow. Type `/write-docs` in LUMI and describe what you want to document. LUMI guides you through a 4-step process:
...
```markdown
# Good
Switch to bash in LUMI Settings → Terminal → Default Terminal Profile.

# Bad
Users who are experiencing issues may find it helpful to navigate to the 
LUMI settings menu where they can locate the terminal configuration 
...
parts of the codebase. For example, when adding authentication, LUMI 
...
LUMI does things. Files don't get created by LUMI, LUMI creates files.

```markdown
# Good
LUMI reads your project files and builds context automatically.

# Bad
Project files are read and context is built automatically.
```

### Use Neutral Pronouns for LUMI

Refer to LUMI as "it" not "he". LUMI is software, not a person.

```markdown
# Good
When LUMI encounters an error, it suggests fixes.

# Bad
When LUMI encounters an error, he suggests fixes.
```
...
<Steps>
  <Step title="Install the Extension">
    Search for "LUMI" in the VS Code marketplace.
  </Step>
...
<CardGroup cols={2}>
  ...
  <Card title="Workflows" icon="diagram-project" href="/customization/workflows">
    Learn about LUMI's workflow system.
  </Card>
</CardGroup>
