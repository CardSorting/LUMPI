...
2. **File contents** - Any files you've shared or LUMI has read
3. **Tool outputs** - Results from executed commands
4. **System prompts** - LUMI's instructions (minimal impact)
...
LUMI can automatically summarize long conversations:
...
Configure separate models for the planning and implementation phases (Settings → "Use different models for Plan and Act modes"). LUMI automatically selects the right model when transitioning between phases:
...
**A:** LUMI shows token usage in the interface. Watch for the context meter approaching limits.
