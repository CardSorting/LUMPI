...
Every interaction with LUMI happens within a **task** — an instance of `Task` in `src/core/task/index.ts`, managed by `Controller` in `src/core/controller/index.ts`.
...
- System instructions that guide LUMI's behavior (including [project rules](/customization/dietcode-rules))

When the context window approaches its limit, use `/compact` or `/smol` to condense history, or `/newtask` to hand off to a fresh task. See [Using commands](/core-workflows/using-commands) and [Memory & reasoning](/MEMORY_AND_REASONING).
...
If your starting context seems high even for simple prompts, add a [`.dietcodeignore`](/customization/dietcodeignore) file to exclude dependencies, build artifacts, and other files LUMI doesn't need.
