...
title: "Hooks"
sidebarTitle: "Hooks"
description: "Inject custom logic into LUMI's workflow to validate operations and shape LUMI's decisions."
---

Hooks are scripts that run at key moments in LUMI's workflow. Because they execute at known points with consistent inputs and outputs, hooks bring determinism to the non-deterministic nature of AI models by enforcing guardrails, validations, and context injection. You can validate operations before they execute, monitor tool usage, and shape how LUMI makes decisions.
...
- Add context to the conversation based on what LUMI is doing

## Hook Types

LUMI supports 8 hook types that run at different points in the task lifecycle:
...
| PreToolUse | Before LUMI executes a tool (read_file, write_to_file, etc.) |
| PostToolUse | After a tool execution completes |
| UserPromptSubmit | When you submit a message to LUMI |
| PreCompact | Before LUMI truncates conversation history to free up context |
...
flowchart TD
    ...
    subgraph Conversation Cycle
        ...
        H_Submit --> Thinking[LUMI Processes Context]:::state
...
2. **Conversation Cycle**: Each time you send a message, **UserPromptSubmit** runs, then LUMI processes your request
3. **Tool Execution**: When LUMI decides to use a tool, **PreToolUse** runs first-if allowed, the tool executes, then **PostToolUse** runs
...
- **Global hooks**: `~/Documents/LUMI/Hooks/`
...
<Steps>
  <Step title="Open the Hooks tab">
    Click the scale icon at the bottom of the LUMI panel, to the left of the model selector. Switch to the Hooks tab.
  </Step>
  ...
  <Step title="Review the hook's code">
    Click the pencil icon to open and edit the hook script. LUMI generates a template with examples.
  </Step>
...
Let's create a simple hook that logs every file LUMI reads or writes. You'll see results in seconds.
...
<Steps>
  <Step title="Create the hook file">
    Save the script above as `~/Documents/LUMI/Hooks/file-logger` or create it through the Hooks UI.
  </Step>
  <Step title="Make it executable">
    On macOS/Linux, run `chmod +x ~/Documents/LUMI/Hooks/file-logger`.
  </Step>
  <Step title="Enable it (macOS/Linux only)">
    In LUMI's Hooks tab, find "file-logger" under PreToolUse hooks and toggle it on.
  </Step>
</Steps>
...
Ask LUMI to read any file in your project: "What's in package.json?"
...
| `contextModification` | string | Optional text that gets injected into the conversation as context for LUMI |
...
- Providing validation results that LUMI should consider
...
Runs when you send a message to LUMI. Use it to:
...
Runs before LUMI truncates conversation history to stay within context limits. Use it to:
