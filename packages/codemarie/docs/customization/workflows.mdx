...
Workflows are Markdown files that define a series of steps to guide LUMI through repetitive or complex tasks. Type `/` followed by the workflow's filename to invoke it (e.g., `/deploy.md`).

Deploying, setting up a new project, running through a release checklist: these tasks often require remembering a dozen steps, running commands in the right order, and updating files manually. Mess up one step and you're debugging for an hour. Workflows turn those multi-step processes into one command. Type `/release.md` and LUMI handles the version bump, runs tests, updates the changelog, commits, tags, and pushes. You just review and approve.
...
- **High-level**: "Run the test suite and fix any failures" lets LUMI decide how to accomplish the goal
...
<Steps>
  <Step title="Open the Workflows menu">
    Click the scale icon at the bottom of the LUMI panel, to the left of the model selector. Switch to the Workflows tab.
  </Step>
...
<Tip>
**Create workflows from completed tasks.** After finishing something you'll need to repeat, tell LUMI: "Create a workflow for the process I just completed." LUMI analyzes the conversation, identifies the steps, and generates the workflow file. Your accumulated context becomes reusable automation.
</Tip>

### Invoking Workflows

Type `/` in the chat input to see available workflows. LUMI shows autocomplete suggestions as you type, so `/rel` would match `release-prep.md`. Select a workflow and press Enter to start it.

LUMI executes each step in sequence, pausing for your approval when needed. You can stop a workflow at any point by rejecting a step.
...
**Global workflows** go in your system's LUMI Workflows directory. Use these for personal productivity workflows you use across all projects.

### Global Workflows Directory

| Operating System | Default Location |
|------------------|------------------|
| Windows | `Documents\LUMI\Workflows` |
| macOS | `~/Documents/LUMI/Workflows` |
| Linux/WSL | `~/Documents/LUMI/Workflows` |
...
Write steps as plain instructions. LUMI interprets them and figures out which tools to use:
...
This approach works well when you want LUMI to adapt to the situation rather than follow rigid steps.

### LUMI Tools

For precise control, use LUMI's built-in tools with XML syntax. This guarantees specific actions:
...
See the full list in the [LUMI Tools Reference](/tools-reference/all-dietcode-tools).
...
Or describe the intent in natural language and let LUMI figure out the tool call:
...
**Include failure handling.** Tell LUMI what to do when something goes wrong: "If tests fail, show the failures and stop the workflow."
...
The workflow demonstrates both approaches: XML tool syntax (`<execute_command>`, `<ask_followup_question>`) for steps that need precise control, and natural language for steps where LUMI should adapt to the situation.
...
Invoke it with `/release-prep.md` and LUMI walks through each step.
