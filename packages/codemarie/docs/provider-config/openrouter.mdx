...
### Unified Token Ingestion Buffer Engine

OpenRouter calls in LUMI are automatically optimized using the **Token Ingestion Buffer Engine** ([token-buffer-engine.ts](/architecture/current)):

- **Explicit Ephemeral Tagging**: Automatically injects `{ cache_control: { type: "ephemeral" } }` onto the system prompt and the last 2 user messages for Claude & MiniMax models via OpenRouter.
- **10-Stage DSL Compression**: Strips comments, collapses stack trace frames, minifies file path bloat (`~.../file.ts`), and transpiles tool call JSON into compact inline DSL syntax (`[tool:name key="val"]`).
- **Context Window Ceiling Guard**: Dynamically trims older middle turns when context window thresholds are reached, preserving system prompt, recent active turns, and user intent.
