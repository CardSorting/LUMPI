...
import { EnvironmentIntegrity } from "../integrity/EnvironmentIntegrity";
...
import { SpiderRefactorer } from "./SpiderRefactorer";
...
export class FluidPolicyEngine {
    ...
    optimizer;
    anomalies;
    buildAlarmActive = false;
    isChecking = false;
    alarmViolations = [];
    stateRestored = false; // V150: Immortality tracking
    lastBuildHealth = 100;
    lastViolationCount = 0;
    telemetrics;
    verification;
    refactorTurnsRemaining = 0; // V70: Stability Refactor Window
    lastEntropyScore = 1.0; // V80: Karma Tracking
    restorationTokens = new Map(); // V100: Recovery Buffers
    gracePeriods = new Map(); // V100: Soft-Lock attempts
    scratchpadSnapshot;
    scratchpadVirtualResolved = false;
    refactorHealer;
    forensics;
    garbageCollector;
    envIntegrity;
    karma = 1000; // High-Velocity: Initial Karma bonus
    /** Last logged velocity — avoids per-tool info spam on guarded pre-exec hot path. */
    lastLoggedActivityVelocity;
    constructor(cwd, streamId, stateManager, virtualResolver) {
        this.cwd = cwd;
        ...
        this.garbageCollector = new IntegrityGarbageCollector(this.cwd, this.spiderEngine, this.anomalies, this.stabilityMonitor, this.usesCanonicalJoyZoning());
        this.telemetrics = new StabilityTelemetrics(this.cwd, this.stabilityMonitor, this.spiderEngine, this.anomalies);
        this.verification = new AxiomVerificationService(this.cwd, this.spiderEngine, this.axiomEngine, this.anomalies, this.forensics);
        this.envIntegrity = new EnvironmentIntegrity(this.cwd, this.stateManager);
        // V16: Warm graph startup
        this.restoreStabilitySubstrate().catch((e) => Logger.error("[FluidPolicyEngine] Failed to restore Stability Substrate:", e));
    }
    /**
     ...
     */
    async incrementStrikes(filePath) {
        if (!this.streamId)
            return 0;
    ...
    setCommitSeal(seal, reason) {
        this.commitSeal = seal;
        this.sealReason = reason;
    }
    getFileLayerContext(filePath) {
     ...
     */
    async recordScanHistory(violations) {
        ...
        await this.dashboardGenerator.updateDashboard(this.spiderEngine, this.auditRecorder, this.stabilityMonitor, this.optimizer, this.anomalies);
        // V150: Substrate Stabilization (Ghost Persistence)
        await this.persistStabilitySubstrate();
    }
    triggerBuildAlarm(violations) {
            ...
            if (allowVirtualFallback && !this.scratchpadVirtualResolved && this.streamId) {
                ...
                if (history) {
                    ...
                    if (virtual.valid) {
                        ...
                        this.scratchpadSnapshot = snapshot;
                        Logger.info("[FluidPolicyEngine] Strategic Interdiction bypassed via Virtual Scratchpad (History Synthesis).");
                        return snapshot;
                    }
                }
            }
     ...
     */
    async validatePreExecution(block) {
        ...
        const isModifying = block.name === DietCodeDefaultTool.FILE_NEW ||
        ...
        if (this.mode === "plan" && isModifying) {
            ...
            if (!isScratchpad) {
                return {
                    success: false,
                    error: `🛑 PLAN MODE RESTRICTION: You are attempting to modify \`${targetPath}\` while in PLAN mode.\n\n` +
        ...
        if (!lease.success || (lease.details?.diskSpaceGB && Number.parseFloat(lease.details.diskSpaceGB) < 0.5)) {
            ...
            if (lease.details?.diskSpaceGB && Number.parseFloat(lease.details.diskSpaceGB) < 0.5) {
                return {
                    success: false,
                    error: `🛑 PHYSICAL BLOCKADE [CRITICAL]: Disk space dangerously low.\n\n` +
            ...
            const isCriticalPhysicalResource = (lease.details?.diskSpaceGB && Number.parseFloat(lease.details.diskSpaceGB) < 0.5) ||
        ...
        if (thrashing.loop &&
            ...
            const isTargetingAudit = block.params?.path?.endsWith("scratchpad.md");
            if (!isTargetingAudit) {
        ...
        if (block.name === DietCodeDefaultTool.FILE_READ) {
            ...
            if (targetPath) {
                ...
                if (violations.length > 0 || status.active) {
                    const alerts = [];
                    if (status.active)
                        alerts.push(`⚠️ HIGH RECENT ACTIVITY: ${status.reason}`);
        ...
        if (block.name === DietCodeDefaultTool.FILE_EDIT || block.name === DietCodeDefaultTool.APPLY_PATCH) {
            ...
            if (targetPath) {
                ...
                const isRefactoring = this.refactorTurnsRemaining > 0 ||
                ...
                if (status.active && isHealingMode && tokens === 0) {
                    ...
                    Logger.info(`[FluidPolicyEngine] Recovery Buffer Activated: 3 restoration tokens granted for ${path.basename(targetPath)}`);
                }
                ...
                if (this.usesCanonicalJoyZoning() && !isScratchpad && block.name === DietCodeDefaultTool.FILE_EDIT) {
                    ...
                    if (content) {
                        ...
                        if (result.status === "NEGATIVE" && !scratchpadContent.includes("# STABILITY_AGILE")) {
                            Logger.info(`[FluidPolicyEngine] Axiomatic Drift detected in ${targetPath} but bypassed in Silent Velocity mode.`);
                        }
                    }
                }
                const hasOverride = block.params.content?.includes("[STABILITY_EXCEPTION: Activity Cooldown Override]");
                const hasBreath = block.params.content?.includes("# STABILITY_RECALIBRATE");
                if (hasBreath) {
                    this.resetSystemPressure();
                ...
                if (status.active && currentTokens > 0) {
                    ...
                    Logger.info(`[FluidPolicyEngine] Recovery Buffer Consumed for ${path.basename(targetPath)} (${currentTokens - 1} remain)`);
                    return {
                        success: true,
        ...
        if (this.usesCanonicalJoyZoning() &&
            ...
            const { path: filePath, content } = block.params;
            if (content) {
                ...
                if (errors.length > 0 && !this.commitSeal) {
                    ...
                    const currentViolations = this.spiderEngine
                        .getViolations()
                        .map((v) => ({ axiom: "STRUCTURAL", severity: "WARN", message: v.message }));
                    ...
                    return {
                        success: true,
                        warning: `⚠️ LOGIC CONSISTENCY WARNING: Logic Integrity has been compromised.\n` +
        ...
        if (block.name === DietCodeDefaultTool.RENAME || block.name === DietCodeDefaultTool.MOVE) {
            const { oldPath, newPath } = block.params;
            ...
            const sim = await this.simulationEngine.simulateMove(oldPath, newPath, this.spiderEngine, this.anomalies, isHealingMode, isAgile);
            if (!sim.safe && !this.commitSeal && !isHealingMode) {
                ...
                return {
                    success: true,
                    [isHealingMode ? "info" : "warning"]: `⚠️ SIMULATION ALERT: ${sim.message}\n` +
        ...
        if (this.stabilityMonitor.getCooldownStatus().active && block.params?.path) {
            const sweepPath = this.normalize(block.params.path);
            Logger.warn(`[FluidPolicyEngine] High Activity Pressure detected. Scheduling deferred Pre-flight Sweep on ${block.params.path}`);
        ...
        if (block.name === DietCodeDefaultTool.FILE_NEW ||
            block.name === DietCodeDefaultTool.FILE_EDIT ||
            block.name === DietCodeDefaultTool.APPLY_PATCH) {
            const { content: scratchpadContent } = await this.resolveScratchpadContext();
        ...
        if ((this.buildAlarmActive || (isCriticalHealth && !isRefactoringWindow)) &&
    ...
    async execute(block) {
        ...
        try {
            ...
            if (block.name === DietCodeDefaultTool.FILE_EDIT &&
                block.params?.path &&
                block.params.diff) {
                const filePath = path.resolve(this.cwd, block.params.path);
            ...
            if ((block.name === DietCodeDefaultTool.FILE_NEW || block.name === DietCodeDefaultTool.FILE_EDIT) &&
                ...
                const astValidation = this.tspPlugin.validateSource(filePath, content, this.virtualResolver, isRecovering || isHealing);
                ...
                if (!astValidation.success) {
                    ...
                    const shouldBlock = !isSovereign &&
                        ((layer === "domain" && projectHealth < 50 && !isHealing) ||
                            (layer === "core" && strikes >= 3 && projectHealth < 60 && !isRecoveringTrend));
                    if (shouldBlock) {
                        const violationSummaryRejection = astValidation.errors.map((e) => `  - ${e}`).join("\n");
                        ...
                        return {
                            ...
                            warning: `🚨 ${rejectionTitle} [RECOVERY_REQUIRED]\n` +
                    ...
                    return {
                        success: true,
                        warning: `📍 [ARCHITECTURAL ADVISORY]: ${layer.toUpperCase()} layer file \`${path.basename(filePath)}\` has ${astValidation.errors.length} violation(s):\n${violationSummary}\n\n` +
                ...
                if (this.spiderEngine.isRecovering) {
                    const isHistoricallyAlarmed = this.alarmViolations.some((v) => v.includes(filePath)) || this.anomalies.hasAnomaly(filePath);
            ...
            if (block.name === DietCodeDefaultTool.FILE_NEW ||
                ...
                const files = params.path ? [path.resolve(this.cwd, params.path)] : [];
                if (files.length > 0) {
            ...
            const isModifyingTool = block.name === DietCodeDefaultTool.FILE_NEW ||
                block.name === DietCodeDefaultTool.FILE_EDIT ||
                block.name === DietCodeDefaultTool.APPLY_PATCH;
            ...
            const targetedPath = block.params?.path;
            if (targetedPath && typeof targetedPath === "string") {
                const advisories = this.spiderEngine.getIntegrityAdvisories(targetedPath);
                const brittlePaths = this.refactorHealer.detectRelativeImports(targetedPath, block.params?.content, this.spiderEngine);
                const missingExports = this.refactorHealer.detectMissingExports(targetedPath, block.params?.content);
                const shadowing = this.refactorHealer.detectShadowing(targetedPath, block.params?.content);
                const unusedImports = this.refactorHealer.detectUnusedImports(targetedPath, block.params?.content);
     ...
     */
    async observeToolOutcome(toolName, output) {
        if (this.mode === "act" &&
    ...
    async onRead(filePath, content, totalReadCount = 0, perFileReadCount = 0, globalFileReadCount = 0) {
        ...
        const sourceFile = require("typescript").createSourceFile(absolutePath, content, require("typescript").ScriptTarget.Latest, true);
        ...
        if (this.mode === "plan") {
            ...
            const isRefactoringIntent = this.refactorTurnsRemaining > 0 || this.spiderEngine.getViolations().length > 0 || this.buildAlarmActive;
            const normPath = this.normalize(absolutePath);
            const nodeSize = this.spiderEngine.nodes.get(normPath)?.astComplexity || 0;
     ...
     */
    async validatePostExecution(block, toolOutput, prevResultHash) {
        const result = { success: true };
                ...
                try {
                    ...
                    try {
                        ...
                        if (sweepResult.fixedCount > 0) {
                            Logger.info(`[FluidPolicyEngine] Sweep fixed ${sweepResult.fixedCount} issues in ${path.basename(filePath)}.`);
                        }
                    }
                    finally {
        ...
        const recoveryDetected = (this.lastBuildHealth < 70 && health > 90) ||
        ...
        if (recoveryDetected) {
            ...
            if (karmaEarned) {
                ...
                Logger.info(`[FluidPolicyEngine] Karma Pardon triggered: Entropy drop ${SafeNumber.formatPercent(entropyDiscovery, 1)}% (+${earned} Karma)`);
            }
     ...
     */
    async validateCommit(affectedFiles, ops) {
        const allErrors = [];
     ...
     */
    getStabilityTelemetry(filePath) {
        const layer = this.getCachedLayer(filePath);
     ...
     */
    async persistStabilitySubstrate() {
        if (!this.streamId)
            return;
        try {
     ...
     */
    async restoreStabilitySubstrate() {
        if (!this.streamId) {
    ...
    getLayerForPath(filePath) {
        const { getLayer } = require("@/utils/joy-zoning");
        return getLayer(filePath);
    }
    async ensureScratchpadIntegrity(taskName = "Architectural Recovery") {
        const scratchpadPath = path.join(this.cwd, "scratchpad.md");
        try {
            const content = await fs.readFile(scratchpadPath, "utf-8");
     ...
     */
    async detectConcurrentDrift(block) {
        if (block.name !== DietCodeDefaultTool.FILE_EDIT && block.name !== DietCodeDefaultTool.APPLY_PATCH)
            return undefined;
        const params = block.params;
        const filePath = params?.path || params?.target_file;
        ...
        for (const [p, m] of registry.entries()) {
            if (m.writes > 0) {
                ...
                if (rel.startsWith(".wiki/"))
                    continue;
                summary.push(`- \`${rel}\` (${m.writes} writes, +${Math.round(m.linesAdded)}/-${Math.round(m.linesDeleted)} lines)`);
            }
        }
     ...
     */
    generateIntegrityAdvisor(files) {
    ...
    async checkForensicCompliance() {
        ...
        if (!metrics || metrics.writes === 0) {
            return {
                compliant: false,
                advisory: "💡 [FORENSIC_ADVISORY]: The Knowledge Ledger (`.wiki/changelog.md`) has not been updated in this session. Consider documenting your changes before completion.",
            };
        }
...
//# sourceMappingURL=FluidPolicyEngine.js.map