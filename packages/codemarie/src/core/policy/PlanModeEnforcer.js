...
export class PlanModeEnforcer {
    ...
    async enforceStrategicReview() {
        ...
        if (!content || content.trim().length === 0) {
            ...
            return {
                ...
                reason: `📍 [STRATEGIC ADVISORY]: Plan Mode is active.\n\n` +
        ...
        const hasSection = (patterns) => lines.some((l) => patterns.some((p) => (typeof p === "string" ? l.includes(p) : p.test(l))));
        ...
        if (!requirementAnalysis)
            missing.push("Requirement Analysis");
        if (!architecturalAnalysis)
            missing.push("Architect review");
        if (!criticAnalysis)
            missing.push("Critic review");
        if (!sreAnalysis)
            missing.push("SRE review");
        if (missing.length > 0) {
            return {
                allowed: true, // V290: Advisory only
                reason: `📍 [STRATEGIC ADVISORY]: Your \`scratchpad.md\` is missing recommended sections: ${missing.join(", ")}.\n` +
        ...
        const reviewersPending = content.includes("[ ] " + IntegrityProtocol.HEADERS.ARCHITECT.replace(/^#+\s+/, "")) ||
            (IntegrityProtocol.SEMANTIC_PATTERNS.ARCHITECT.test(content) && content.includes("[ ]"));
        if (reviewersPending && !isSurgical) {
            return {
                allowed: true, // V290: Advisory only
                reason: `📍 [STABILITY ADVISORY]: STABILITY GUARD markers ([ ]) are still pending in your \`scratchpad.md\`.\n` +
     ...
     */
    async getStrategicReviewStatus(monitor) {
        const content = await this.readScratchpad();
        if (!content) {
            return {
                hasScratchpad: false,
        ...
        return {
            ...
            Architect: lines.some((l) => l.includes(IntegrityProtocol.HEADERS.ARCHITECT) || IntegrityProtocol.SEMANTIC_PATTERNS.ARCHITECT.test(l)),
            Critic: lines.some((l) => l.includes(IntegrityProtocol.HEADERS.CRITIC) || IntegrityProtocol.SEMANTIC_PATTERNS.CRITIC.test(l)),
            SRE: lines.some((l) => l.includes(IntegrityProtocol.HEADERS.SRE) || IntegrityProtocol.SEMANTIC_PATTERNS.SRE.test(l)),
            TRIADAudit: !content.includes("[ ] " + IntegrityProtocol.HEADERS.ARCHITECT.replace(/^#+\s+/, "")) &&
     ...
     */
    async updateScratchpadWithFeasibility(feedback) {
     ...
     */
    checkExplorationDepth(fileReadCount) {
        const thresholds = {
            shallow: 5,
            adequate: 10,
        };
        if (fileReadCount < thresholds.shallow)
            return "shallow";
        if (fileReadCount < thresholds.adequate)
            return "adequate";
        return "overdetailed";
    }
    /**
     ...
     */
    async generateStrategicReviewPrompts() {
        ...
        if (!status.Requirements)
            missing.push("Requirement Analysis");
        if (!status.Architect)
            missing.push(IntegrityProtocol.HEADERS.ARCHITECT.replace(/^#+\s+/, "") + " review");
        if (!status.Critic)
            missing.push(IntegrityProtocol.HEADERS.CRITIC.replace(/^#+\s+/, "") + " review");
        if (!status.SRE)
            missing.push(IntegrityProtocol.HEADERS.SRE.replace(/^#+\s+/, "") + " review");
        if (!status.TRIADAudit)
            missing.push("STABILITY GUARD ([x] marks)");
        if (missing.length === 0) {
     ...
     */
    performArchitectAudit(planSummary) {
        const issues = [];
     ...
     */
    performCriticAudit(planSummary) {
        const issues = [];
     ...
     */
    checkLayerDiscipline(planSummary) {
        const violations = [];
...
//# sourceMappingURL=PlanModeEnforcer.js.map