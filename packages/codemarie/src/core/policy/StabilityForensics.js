...
export class StabilityForensics {
    ...
    stabilityMonitor;
    spiderEngine;
    constructor(cwd, stabilityMonitor, spiderEngine) {
        this.cwd = cwd;
     ...
     */
    async verifyEvidenceVerification(content, history = []) {
        const errors = [];
        const warnings = [];
        ...
        const citedPaths = Array.from(new Set(Array.from(content.matchAll(pathRegexp)).map((m) => (m[0].startsWith("/") ? m[0].slice(1) : m[0]))));
        for (const cited of citedPaths) {
            ...
            if (!metrics || metrics.reads === 0) {
                ...
                if (historyPaths.has(absoluteCited)) {
                    warnings.push(`💡 CONVERSATIONAL GROUNDING: \`${cited}\` was discussed in recent dialogue. Grounding assumed via neural context.`);
                    continue;
                }
                ...
                if (node) {
                    warnings.push(`💡 INVESTIGATION PERSISTENCE: \`${cited}\` cited without recent read, but structural identity is verifiably stable. Verification assumed.`);
                    continue;
                }
                warnings.push(`⚠️ UNVERIFIED FILE CITATION: You cited \`${cited}\` but I don't see a recent read history for it. Please take a look at the file so we can stay in sync!`);
                continue;
            }
            ...
            if (metrics.lastObservedHash && !isRecentlyRead) {
                try {
                    ...
                    if (currentHash !== metrics.lastObservedHash) {
                        warnings.push(`⚠️ RECENT FILE CHANGES DETECTED: \`${cited}\` has been updated since your last look. ` +
            ...
            if (stalenessMs > 1200000) {
                warnings.push(`⚠️ OLD OBSERVATION: Your latest look at \`${cited}\` is over 20 minutes old. It might be worth a quick re-check to ensure nothing has changed.`);
            }
        }
        ...
        for (const symbol of citedSymbols) {
            ...
            if (!observed) {
                ...
                warnings.push(`💡 UNVERIFIED SYMBOL: \`${symbol}\` cited but not explicitly logged as observed. Ensure your strategic review probes are linked to verified symbols.`);
            }
        }
     ...
     */
    generateInvestigationTrace() {
        const registry = this.stabilityMonitor.getForensicRegistry();
        const trace = [];
        const recentEntries = Array.from(registry.entries())
     ...
     */
    extractPathsFromHistory(history) {
        const paths = new Set();
        ...
        for (const msg of assistantTurns) {
            const text = Array.isArray(msg.content)
                ? msg.content
                    .map((c) => c.text || JSON.stringify(c.input || {}))
                    .join(" ")
                : String(msg.content);
     ...
     */
    verifyBlockStability(currentContent, targetContent) {
...
//# sourceMappingURL=StabilityForensics.js.map