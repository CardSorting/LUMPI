...
import type { SpiderEntropyReport, SpiderNode, SpiderSnapshot } from "./types";
...
export class MetricsEngine {
	...
	public detectRefactoringFatigue(node: SpiderNode, pressure: number, snapshots: SpiderSnapshot[]): boolean {
		...
		const variance = complexities.reduce(
			(a, b) => a + (b - complexities.reduce((x, y) => x + y, 0) / history.length) ** 2,
			0,
		);
	...
	public computeEntropy(nodes: Map<string, SpiderNode>, history: number[] = []): SpiderEntropyReport {
		...
		const rawScore =
			depthScore * 0.1 + namingScore * 0.15 + orphanScore * 0.15 + couplingScore * 0.4 + cognitiveScore * 0.2;
		...
		for (let i = 0; i < nodesArray.length; i++) {
			for (let j = i + 1; j < nodesArray.length; j++) {
				...
				const complexityDiff =
					Math.abs(a.astComplexity - b.astComplexity) / Math.max(a.astComplexity, b.astComplexity);
