...
const finiteNodeNumber = (value, fallback = 0) => typeof value === "number" && Number.isFinite(value) ? value : fallback;
/**
...
export class SpiderEngine {
    ...
    ghosts = new Set();
    version = 0;
    isRecovering = false; // Track if the last operation improved project health
    /**
    ...
    sessionBuffer = new Map(); // V71: Virtual Session Sensing
    stabilityLock = null; // V190: Lock owner
    stabilityLockId = null; // V200: Session-specific lock ID
    stabilityHeartbeat = null; // V190: Lock expiration timer
    substrateCheckpoint = null; // V200: Resilience Snapshot
    checkpointTimestamp = null; // V200: Snapshot metadata
     ...
     */
    async acquireStabilityLock(owner, sessionId) {
    ...
    releaseStabilityLock(owner, lockId) {
     ...
     */
    computeActivityPressure(monitor) {
    ...
    async warmUp(entryPoints = ["src/main.ts", "src/index.ts"]) {
    ...
    updateNode(filePath, content, skipResolution = false) {
    ...
    extractExports(sourceFile) {
        const result = { symbols: [], reExports: [] };
            ...
            if (ts.isVariableStatement(node)) {
                for (const decl of node.declarationList.declarations) {
                    if (ts.isIdentifier(decl.name))
                        result.symbols.push(decl.name.text);
                }
            }
    ...
    visitDetailedImports(node, imports) {
        if (ts.isImportDeclaration(node) && ts.isStringLiteral(node.moduleSpecifier)) {
            ...
            imports.push({ specifier: node.arguments[0].text, symbols: ["*"] });
        }
    ...
    visitMetrics(node, ctx, depth) {
        ...
        ts.forEachChild(node, (child) => {
            const isNesting = kind === ts.SyntaxKind.IfStatement ||
                kind === ts.SyntaxKind.ForStatement ||
                kind === ts.SyntaxKind.WhileStatement ||
                kind === ts.SyntaxKind.SwitchStatement ||
    ...
    checkDeepAny(typeNode, ctx) {
        ...
        ts.forEachChild(typeNode, (child) => {
            if (ts.isTypeNode(child))
                this.checkDeepAny(child, ctx);
        });
    }
    detectInterface(path, sourceFile) {
        let hasConcrete = false;
        const visit = (node) => {
            if (ts.isClassDeclaration(node) || ts.isFunctionDeclaration(node) || ts.isVariableDeclaration(node)) {
                const hasBody = (ts.isFunctionDeclaration(node) && node.body) ||
    ...
    updateIncrementalCoupling(nodeId, oldImports, newImports) {
     ...
     */
    async verifySubstrateIntegrity() {
        ...
        if (currentMerkle !== freshMerkle) {
            ...
            Logger.warn(`[SpiderEngine] Substrate Drift Detected! Merkle ${currentMerkle.substring(0, 8)} -> ${freshMerkle.substring(0, 8)}`);
            return { synchronized: false, drift };
        }
        return { synchronized: true, drift: 0 };
    }
    checkStabilityPressure() {
        const stats = v8.getHeapStatistics();
        const usedPercent = (stats.used_heap_size / stats.heap_size_limit) * 100;
        if (usedPercent > 90) {
            Logger.error(`[SpiderEngine] CRITICAL Activity Pressure (${SafeNumber.format(usedPercent, 1)}%). Triggering ABSOLUTE SWEEP.`);
            this.resolver.dispose();
            this.sessionBuffer.clear();
        ...
        if (usedPercent > 80) {
            Logger.warn(`[SpiderEngine] High Activity Pressure detected (${SafeNumber.format(usedPercent, 1)}%). Triggering Selective Sweep.`);
        ...
        for (const node of this.nodes.values()) {
            ...
            if (drift) {
                violations.push({
                    ...
                    path: node.path,
                    message: drift,
                    remediation: "Audit the new vocabulary. If these symbols represent a new domain, consider a domain-level fission.",
                });
            }
            // Refactoring Fatigue
            ...
            if (this.metrics.detectRefactoringFatigue(node, pressure, snapshots)) {
                violations.push({
                    ...
                    remediation: "Fundamental Rethink Required: The current module design is resisting changes. Consider a complete architectural redesign of this component.",
                });
            }
        }
     ...
     */
    getIntegrityAdvisories(filePath) {
        const advisories = [];
        ...
        for (const cycle of this.cachedCycles) {
            if (filePath && !cycle.includes(this.resolver.normalizePath(filePath)))
                continue;
            ...
            if (node && node.afferentCoupling > 5) {
                ...
                if (newContent) {
                    ...
                    if (removed.length > 0) {
                        ...
                        const dependentList = sampleDependents.length > 0
    ...
    addSuppression(violationId, path, message) {
     ...
     * Finds symbols in the substrate that are lexicographically similar to the target.
     */
    findSimilarSymbols(symbol, limit = 3) {
     ...
     */
    async loadRegistry(data) {
        if (data) {
            try {
                if (typeof data === "string") {
                    const parsed = JSON.parse(data);
                    if (!Array.isArray(parsed))
                        throw new Error("Invalid string substrate format.");
                    this.nodes = new Map(parsed);
                }
                else {
                    const payload = this.persistence.deserialize(data);
                    if (!payload || !payload.nodes)
                        throw new Error("Invalid binary substrate payload.");
                    this.nodes = new Map(payload.nodes);
                }
    ...
    async rebuildRegistry(onProgress, options = {}) {
    ...
    async performRegistryRebuild(onProgress, options = {}) {
        if (this.isIndexing) {
            ...
            for (let i = 0; i < files.length; i += BATCH_SIZE) {
                ...
                for (const f of batch) {
                    ...
                    try {
                        ...
                        let exports = exportsData.symbols;
     ...
     */
    async synchronizeRegistry(pressureMap = new Map()) {
        let pruned = 0;
        let reindexed = 0;
    ...
    computeCCI(filePath, anomalies, monitor) {
     ...
     */
    predictMissingImports(filePath, content) {
        ...
        const visit = (node) => {
            if (ts.isImportDeclaration(node)) {
                if (node.importClause) {
                    if (node.importClause.name)
                        imported.add(node.importClause.name.text);
                    if (node.importClause.namedBindings) {
                        if (ts.isNamedImports(node.importClause.namedBindings)) {
     ...
     */
    calculateNamingScore(sourceFile) {
        let total = 0;
        let valid = 0;
...
//# sourceMappingURL=SpiderEngine.js.map