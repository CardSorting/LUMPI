...
export class ModuleDecomposer {
	...
	public analyze(
				...
				if (body) {
					...
					if (layer === "infrastructure" && density > 0.3 && !hasIO) {
						steps.push({
							...
							risk: "MEDIUM",
							reason:
								"This logic is purely computational (high density, no I/O) and should live in the Domain layer.",
			...
			if (ts.isTypeAssertionExpression(node) || ts.isAsExpression(node)) {
				...
				if (checkDeepAny(type)) {
					steps.push({
						...
						risk: "HIGH",
						reason:
							"Production Risk: 'any' keyword detected within type cast. This bypasses the integrity of the substrate.",
					});
				}
			}
			...
			if (ts.isIfStatement(n) || ts.isForStatement(n) || ts.isSwitchStatement(n)) {
				if (depth > 4) {
					steps.push({
						...
						risk: "LOW",
						reason:
							"Shadow Complexity: Deep nesting detected (depth > 4). Extract the inner logic to a private method.",
					});
				}
		...
		const { projectedHealth, projectedIntegrity } = this.calculateProjectedMetrics(
			plan,
			totalLines,
			steps,
			sourceFile,
	...
	private buildLocalSymbolGraph(
		sourceFile: ts.SourceFile,
	): Record<string, { dependents: string[]; dependencies: string[] }> {
	...
	private calculateProjectedMetrics(
		...
		const projectedIntegrity = Math.max(
			0,
			plan.integrityScore + Math.round((100 - plan.integrityScore) * resolvedRatio),
		);
