...
export class JoyRideCache {
	...
	trySet(key: string, value: unknown, metadata: JoyRideSetMetadata): JoyRideSetResult {
		...
		const estimatedBytes = Math.ceil(
			(metadata.estimatedBytes ?? this.estimateValueBytes(value)) * SIZE_ESTIMATE_OVERHEAD,
		);
	...
	private validateEntry(entry: JoyRideCacheEntry<unknown>, fingerprint: JoyRideValidationFingerprint): boolean {
		...
		if (
			fingerprint.workspaceFingerprint !== undefined &&
		...
		if (
			fingerprint.lockfileFingerprint !== undefined &&
	...
	private pruneStaleDiagnostics(): void {
		const staleEntries = [...this.entries.entries()].filter(
			([, entry]) => entry.staleReason || this.isExpired(entry),
		);
	...
	private isVerificationValidationComplete(
		...
		const requiredValidationFields: (keyof JoyRideValidationFingerprint)[] = [
			...requiredEntryFields,
			"relevantFileHashes",
		];
