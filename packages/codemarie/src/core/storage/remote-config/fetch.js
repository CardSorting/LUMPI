...
import { APIKeySchema, RemoteConfigSchema, } from "../../../shared/remote-config/schema";
 ...
 */
async function makeAuthenticatedRequest(endpoint, organizationId) {
...
async function fetchRemoteConfigForOrganization(organizationId) {
    try {
        ...
        const configData = await makeAuthenticatedRequest(DIETCODE_API_ENDPOINT.REMOTE_CONFIG, organizationId);
 ...
 */
async function fetchApiKeysForOrganization(organizationId) {
    try {
        // Fetch API keys string using helper
        const response = await makeAuthenticatedRequest(DIETCODE_API_ENDPOINT.API_KEYS, organizationId);
