...
function accessSettingsToBlobStorage(type, settings) {
    return {
        adapterType: type,
...
async function applyRemoteOTELConfig(transformed, telemetryService) {
    try {
        ...
        if (isOpenTelemetryConfigValid(otelConfig)) {
            ...
            if (client.meterProvider || client.loggerProvider) {
                telemetryService.addProvider(await new OpenTelemetryTelemetryProvider(client.meterProvider, client.loggerProvider, {
...
async function applyRemoteSyncQueueConfig(transformed) {
    try {
 ...
 */
export async function applyRemoteConfig(remoteConfig, configuredKeys, mcpHub) {
    const stateManager = StateManager.get();
...
const isProviderValid = (provider, remoteConfig) => {
    const remoteConfiguredProviders = remoteConfig?.remoteConfiguredProviders ?? StateManager.get().getRemoteConfigSettings().remoteConfiguredProviders;
 ...
 */
function filterAllowedRemoteConfigFieldsLegacy(config) {
...
const canDisableRemoteConfig = (orgId) => {
...
export const isRemoteConfigEnabled = (orgId) => {
    const stateManager = StateManager.get();
