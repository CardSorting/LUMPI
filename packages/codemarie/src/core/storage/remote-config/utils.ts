...
function accessSettingsToBlobStorage(
	type: BlobStoreSettings["adapterType"],
...
export async function applyRemoteConfig(
	...
	const syncedWorkflowToggles = synchronizeRemoteRuleToggles(
		remoteConfig.globalWorkflows || [],
		currentWorkflowToggles,
	);
 ...
 */
function filterAllowedRemoteConfigFieldsLegacy(
	config: Partial<GlobalStateAndSettings>,
): Partial<GlobalStateAndSettings> {
	return filterAllowedRemoteConfigFields(
		config,
		StateManager.get().getRemoteConfigSettings().remoteConfiguredProviders,
	);
}
