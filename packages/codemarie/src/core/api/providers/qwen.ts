...
export class QwenHandler implements ApiHandler {
	...
	getModel(): { id: MainlandQwenModelId | InternationalQwenModelId; info: ModelInfo } {
		...
		if (this.useChinaApi()) {
			const id =
				modelId && modelId in mainlandQwenModels ? (modelId as MainlandQwenModelId) : mainlandQwenDefaultModelId;
			return {
				id,
	...
	async *createMessage(systemPrompt: string, messages: DietCodeStorageMessage[], tools?: OpenAITool[]): ApiStream {
		...
		const isReasoningModelFamily =
			model.id.includes("qwen3") || ["qwen-plus-latest", "qwen-turbo-latest"].includes(model.id);
