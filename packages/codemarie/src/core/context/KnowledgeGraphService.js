...
import { getDb, registerDbPathChangeListener } from "../../infrastructure/db/Config";
export class KnowledgeGraphService {
    static instance = null;
    embeddingHandler;
    cleanupInterval = null;
    isEmbeddingDisabled = false;
    unregisterDbListener = null;
    constructor(embeddingHandler) {
    ...
    static async getInstance(embeddingHandler) {
     ...
     */
    async appendSharedMemory(streamId, memory) {
        const stream = await dbPool.selectOne("agent_streams", [
     ...
     */
    async getSharedMemory(streamId) {
        const memories = [];
    ...
    async addKnowledge(streamId, type, content, options = {}) {
        const id = nanoid();
     ...
     */
    async cognitiveSnapshot(streamId, content, count) {
        const id = nanoid();
        let landmarkId;
        if (count > 20) {
     ...
     */
    async appendMemoryLayer(streamId, key, memory) {
        const db = await getDb();
     ...
     */
    async createLandmark(streamId, content, originalCount) {
        // Landmark summarized using generic ApiHandler is handled elsewhere or we fallback to Gemini summarize if available
        const summary = this.embeddingHandler.summarizeText
            ? await this.embeddingHandler.summarizeText(content)
    ...
    async traverseGraph(startId, maxDepth = 2, filter) {
     ...
     */
    async searchKnowledge(streamId, query, options = {}) {
        ...
        const ranked = nodes
            .map((n) => {
            const embedding = n.embedding ? JSON.parse(n.embedding) : null;
     ...
     */
    async extractSubgraph(rootId, maxDepth = 2, filter) {
     ...
     */
    async decayConfidence(factor, olderThanMs) {
        const db = await getDb();
        const rows = await db
            .selectFrom("agent_knowledge")
     ...
     */
    async getHistory(streamId, limit = 200) {
        const db = await getDb();
     ...
     */
    async getContextGraph(streamId, filePath, limit = 50) {
     ...
     */
    async calculateBlastRadius(streamId, filePath, maxDepth = 2) {
     ...
     */
    async detectChokepoints(streamId, limit = 10) {
     ...
     */
    async recoverFile(streamId, filePath) {
        const history = await this.getHistory(streamId, 300);
     ...
     */
    async generateChangelog(streamId, baseId, headId) {
        ...
        if (!headNode || !baseNode)
            return "Base or Head snapshot not found in history.";
...
`;
     ...
     */
    async simulateMerge(sourceStreamId, targetStreamId) {
     ...
     */
    async simulateMergeForecast(sourceStreamId, targetStreamId) {
        ...
        for (const path of mergeSim.affectedPaths) {
            ...
            for (const item of radius)
                sourceRadii.set(item.path, item.depth);
        }
        // 2. Identify files changed in the target stream
        const targetHistory = await this.getHistory(targetStreamId, 50);
        const targetChangedPaths = new Set();
        ...
        for (const path of targetChangedPaths) {
            ...
            for (const item of radius)
                targetRadii.set(item.path, item.depth);
        }
     ...
     */
    async spawnTask(streamId, description, complexity = 1.0, linkedKnowledgeIds = []) {
        const id = nanoid();
        await this._push({
            type: "insert",
            table: "agent_tasks",
    ...
    async getTaskContext(taskId) {
        try {
            ...
            if (linkedKnowledgeIds.length > 0) {
                const graphPromises = linkedKnowledgeIds.map((kbId) => this.traverseGraph(kbId, 2));
