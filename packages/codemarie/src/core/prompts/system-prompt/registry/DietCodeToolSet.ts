...
export class DietCodeToolSet {
	...
	private static getDynamicSubagentToolSpecs(
		...
		const requestedIds = variant.tools ? [...variant.tools] : [];
		const shouldIncludeSubagentTools =
			requestedIds.length === 0 || requestedIds.includes(DietCodeDefaultTool.USE_SUBAGENTS);
