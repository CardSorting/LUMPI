...
import { WORKSPACE_KNOWLEDGE_CATEGORIES, } from "./types";
...
export class WorkspaceIntelligenceEngine {
    ...
    async learnFromFinalization(input) {
        ...
        if (sourceSnapshot.architecturalSurfaces.includes("src/") &&
        ...
        try {
            ...
            try {
                ...
                const entry = {
                    ...
                    recoveryHints: errMsg.toLowerCase().includes("permission") || errMsg.toLowerCase().includes("eacces")
                        ? [
                ...
                await appendFile(path.join(this.config.cwd, ".wiki/intelligence/diagnostics.jsonl"), `${JSON.stringify(entry)}\n`, "utf-8");
            }
            catch {
    ...
    async publishToCognitiveMemory(model, input) {
        const knowledgeGraphService = this.config.services?.knowledgeGraphService;
...
async function collectSourceSnapshot(cwd) {
...
async function detectDrift(cwd, snapshot, previousModel) {
    const findings = [];
...
function addSignal(categories, signal) {
...
function carryForwardDurableSignals(previousModel, categories) {
    if (!previousModel)
        return;
    for (const category of WORKSPACE_KNOWLEDGE_CATEGORIES) {
        ...
        for (const signal of previousModel.categories[category] ?? []) {
            if (categories[category].some((item) => item.id === signal.id))
                continue;
...
async function detectManifests(cwd) {
    const found = [];
...
async function detectDocumentationFiles(cwd) {
    const docs = [
...
async function detectArchitecturalSurfaces(cwd) {
    const found = [];
...
async function detectProviderKeys(cwd) {
...
async function detectToolCount(cwd) {
...
async function detectReadOnlyToolCount(cwd) {
    const text = await readFileIfExists(path.join(cwd, "src/shared/tools.ts"));
    if (!text)
        return undefined;
...
function normalizeWorkspaces(workspaces) {
    if (Array.isArray(workspaces)) {
        return workspaces.filter((workspace) => typeof workspace === "string").sort();
    }
...
function selectPreferredCommands(packageScripts) {
...
function extractImpactFiles(impactSummary) {
    const files = new Set();
...
function extractProviderCounts(readme) {
    const words = new Map([
        ["one", 1],
        ["two", 2],
        ["three", 3],
    ...
    for (const match of readme.matchAll(/\b(\d+|one|two|three|four|five|six|seven|eight|nine|ten)\s+(?:active\s+)?providers?\b/gi)) {
...
function buildKnownUnknowns(snapshot, driftFindings) {
    const unknowns = [];
    if (!snapshot.preferredCommands.length) {
...
function buildHighRiskSurfaces(snapshot, changedFiles) {
    const highRisk = new Set();
    for (const file of changedFiles) {
        if (file.startsWith("src/core/task/tools/finalization/"))
            highRisk.add("finalization lifecycle");
        if (file.startsWith("src/shared/completion/"))
            highRisk.add("completion receipt contract");
        if (file.startsWith("src/core/api/"))
            highRisk.add("provider dispatch");
        if (file.startsWith("webview-ui/"))
            highRisk.add("webview UI");
        if (file.startsWith("broccolidb/"))
            highRisk.add("BroccoliDB substrate");
    }
...
function buildMetaReflection(snapshot, changedFiles, driftFindings) {
    const repeatedFriction = [];
    const rediscoveryCosts = [];
...
function buildCognitiveMemorySummary(model) {
...
async function buildFacts(cwd, snapshot, changedFiles, driftFindings, input, previousModel) {
    const currentFacts = [];
    // 1. Stable Subsystems
            ...
            provenance: [
                {
                    type: "finalization_evidence",
                    runId: input.finalizationRunId,
                    description: "Task modified both the VS Code extension host (src/) and the webview interface (webview-ui/), requiring separate validation pipelines.",
                    timestamp: input.timestamp,
                },
            ],
            lifecycle: "active",
...
export function mergeAndLifecycleManageFacts(currentFacts, previousFacts, input) {
    const currentFactsMap = new Map();
    ...
    const merged = [...deduplicatedCurrent];
    for (const prev of previousFacts) {
        ...
        if (prev.type === "subsystem_stability") {
            const currentConflicting = deduplicatedCurrent.find((f) => f.type === "subsystem_stability" &&
                f.value?.path === prev.value?.path);
                ...
                merged.push({
                    ...
                    provenance: [
                        ...prev.provenance,
                        {
                            type: "finalization_evidence",
                            runId: input.finalizationRunId,
                            description: `Fact superseded because subsystem stability status changed to ${currentConflicting.value?.status} in task ${input.taskId}.`,
                            timestamp: input.timestamp,
                        },
                    ],
                });
        ...
        if (prev.type === "architecture_decision") {
            const currentConflicting = deduplicatedCurrent.find((f) => f.type === "architecture_decision" &&
                f.value?.id === prev.value?.id &&
                f.value?.status !== prev.value?.status);
            if (currentConflicting) {
                merged.push({
                    ...prev,
                    lifecycle: "superseded",
                    ...
                    provenance: [
                        ...prev.provenance,
                        {
                            type: "finalization_evidence",
                            runId: input.finalizationRunId,
                            description: `Decision status changed to ${currentConflicting.value?.status} in task ${input.taskId}.`,
                            timestamp: input.timestamp,
                        },
                    ],
                });
...
async function parseArchitectureDecisions(cwd) {
...
//# sourceMappingURL=WorkspaceIntelligenceEngine.js.map