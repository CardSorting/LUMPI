...
import {
	type JoyZoningBatchRefactorRequest,
			...
			for (const req of groupItems) {
				...
				if (fs.existsSync(absPath)) {
					...
					const step = plan.steps.find((s) => {
						...
						return (
							stepAction === req.action || (!!stepTarget && `${stepAction}: ${stepTarget}`.includes(req.action))
						);
					});
 ...
 */
function sortRequestsBySymbolDependency(
	requests: JoyZoningRefactorRequest[],
	engine: SpiderEngine,
): JoyZoningRefactorRequest[] {
	const validRequests = requests.filter(
		(request) => isNonEmptyString(request.action) && isNonEmptyString(request.path),
	);
 ...
 */
function sortRequestsByDependency(
	requests: JoyZoningRefactorRequest[],
	engine: SpiderEngine,
): JoyZoningRefactorRequest[] {
