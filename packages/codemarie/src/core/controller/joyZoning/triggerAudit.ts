...
import type { SpiderNode } from "@core/policy/spider/SpiderEngine";
import {
	type JoyZoningAuditRequest,
...
const finitePercent = (value: unknown, fallback = 0): number =>
	Math.max(0, Math.min(100, finiteNumber(value, fallback)));
...
const normalizeHistory = (
	value: unknown,
): { timestamp: string; health: number; stability: number; maintainability: number }[] =>
...
const normalizeViolations = (value: unknown): JoyZoningViolation[] =>
	boundedArray<unknown>(value, 500).map(normalizeViolation);
...
function normalizeAuditResponse(
	input: Partial<JoyZoningAuditResponse> | Record<string, unknown>,
): JoyZoningAuditResponse {
...
export async function triggerAudit(
	...
	try {
		...
		const sanitizeScore = (v: unknown) =>
			finitePercent(typeof v === "number" && !Number.isNaN(v) ? Math.round(v) : 0);
		...
		const finalResponse = normalizeAuditResponse({
			...
			driftCount: 0,
			structuralSinks:
				toxicHeatmap.length > 0 ? toxicHeatmap : monitor.getStabilityStats().hotspots.map((h) => h.path),
