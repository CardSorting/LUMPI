...
import { PromptRegistry } from "../prompts/system-prompt/index.js";
import { ensureCacheDirectoryExists, ensureMcpServersDirectoryExists, ensureSettingsDirectoryExists, GlobalFileNames, writeMcpMarketplaceCatalogToCache, } from "../storage/disk.js";
import { fetchRemoteConfig } from "../storage/remote-config/fetch.js";
import { clearRemoteConfig } from "../storage/remote-config/utils.js";
import { StateManager } from "../storage/StateManager.js";
import { Task } from "../task/index.js";
import { disposeRequestRegistry } from "./grpc-handler.js";
import { sendMcpMarketplaceCatalogEvent } from "./mcp/subscribeToMcpMarketplaceCatalog.js";
import { appendDietCodeStealthModels } from "./models/refreshOpenRouterModels.js";
import { disposeAllPersistentSubscriptionHubs } from "./persistent-subscription-hub.js";
import { sendStateUpdate } from "./state/subscribeToState.js";
import { sendChatButtonClickedEvent } from "./ui/subscribeToChatButtonClicked.js";
export class Controller {
    ...
    spider;
    spiderInitPromise;
    async getSpiderEngine() {
        if (this.spider)
            return this.spider;
        if (this.spiderInitPromise)
    ...
    */
    ...
    async initTask(task, images, files, historyItem, taskSettings, initialTaskState) {
    ...
    async reinitExistingTaskFromId(taskId, initialState) {
    ...
    async switchToPlanModeForAgent() {
        return this.switchAgentMode("plan");
    }
    async switchAgentMode(modeToSwitchTo) {
    ...
    async togglePlanActMode(modeToSwitchTo, _chatContent) {
    ...
    async handleAuthCallback(customToken, provider = null, state = null) {
        try {
            await this.authService.handleAuthCallback(customToken, provider ? provider : "google", state);
            const dietcodeProvider = "dietcode";
    ...
    async importTask(importData) {
        ...
        // Write files
        await fs.writeFile(path.join(taskDirPath, GlobalFileNames.apiConversationHistory), JSON.stringify(apiConversationHistory, null, 2));
    ...
    async getStateToPostToWebview() {
        ...
        const auditIntentThresholdAdjustmentsEnabled = this.stateManager.getGlobalSettingsKey("auditIntentThresholdAdjustmentsEnabled");
