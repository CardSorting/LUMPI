	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		try {
			...
			const addItem = (
				...
				if (!diskPath) {
					...
					risks.push({
						...
						level: "warning",
						reason:
							"The map references this file, but it was not found on disk. Verify the graph is fresh before relying on it.",
						mitigation:
							"Run an exact file search/read check before including this path in the implementation plan.",
					});
				}
			};
			...
			if (targetNode) {
				...
				evidence.push({
					type: "disk",
					description: `Suggested reading ${targetNode.path} as the primary fact check.`,
				});
			} else if (pathParam) {
				staleGraph = true;
				addRisk({
					...
					level: "warning",
					reason:
						"Requested path was not present in the project map. Use disk reads/searches to confirm whether it exists or the map is stale.",
			...
			if (symbolParam) {
				...
				evidence.push({
					type: "spider",
					description: `Found ${providers.length} provider(s) for '${symbolParam}'.`,
				});
			...
			if (!targetNode && !symbolParam && queryParam) {
				...
				evidence.push({
					type: "spider",
					description: `Matched query tokens against file paths and exported symbols.`,
				});
			}
				...
				JSON.stringify(
					{
						title: "Project Map",
						summary:
							"Use this map first, then verify with the suggested searches/reads before presenting a plan.",
