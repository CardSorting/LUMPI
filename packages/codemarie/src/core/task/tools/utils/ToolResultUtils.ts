...
export class ToolResultUtils {
		...
		if (typeof content === "string") {
			...
			if (
				userMessageContent.some(
					(item) => item.type === "tool_result" && item.tool_use_id === toolUseId && item.content,
				)
			) {
				Logger.warn(
					`ToolResultUtils: Tool result for tool_use_id ${toolUseId} already exists. Skipping duplicate.`,
				);
				return;
			}
	...
	static pushAdditionalToolFeedback(
		...
		const content = formatResponse.toolResult(
			feedbackText,
			images,
			hasMeaningfulFileContent ? fileContentString : undefined,
		);
		if (typeof content === "string") {
			userMessageContent.push({
				type: "text",
