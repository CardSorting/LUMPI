...
describe("SubagentBuilder", () => {
	...
	it("applies plan-mode openrouter model override fields", () => {
		...
		assert.equal(
			(effectiveApiConfig as Record<string, unknown>).planModeOpenRouterModelId,
	...
	it("builds native tools by filtering allowed ids and context requirements then converting", () => {
		...
		const getModelFamilyStub = sinon
			.stub(PromptRegistry.getInstance(), "getModelFamily")
			.returns("test-family" as never);
