...
import { type EmbeddingHandler, KnowledgeGraphService } from "../context/KnowledgeGraphService";
...
export class Task {
		...
		if (record.state === "suspended") {
			try {
				...
				if (
					...
					record.cause.reason ===
						`awaiting_completion_decision:${latestRejection.completionAttemptId as string}` &&
	...
	private async commitResumeLifecycle(): Promise<TaskLifecycleRecord> {
		const authority = this.lifecycleAuthority();
		const current =
			authority.readProjection(this.taskState) ?? (await authority.restore(this.taskState, this.taskId));
	...
	async abortTask(options?: { reason?: string }) {
		...
		let lifecycleRecord =
			lifecycle.readProjection(this.taskState) ?? (await lifecycle.restore(this.taskState, this.taskId));
		if (!lifecycleRecord) {
	...
	async executeCommandTool(
		...
		const scope = createJoyRideTaskScope(
			this.taskId,
			this.cwd,
			this.terminalExecutionMode,
	...
	async *attemptApiRequest(previousApiReqIndex: number): ApiStream {
		...
		const promptContext: SystemPromptContext = {
			...
			dietcodeWebToolsEnabled:
				this.stateManager.getGlobalSettingsKey("dietcodeWebToolsEnabled") &&
			...
			try {
				...
				if (kgService && contextManagementMetadata.conversationHistoryDeletedRange) {
					...
					if (truncatedRange.length > 0) {
						...
						const snapshotId = await kgService.cognitiveSnapshot(
							this.taskId,
							snapshotContent,
	...
	async recursivelyMakeDietCodeRequests(
		...
		if (!eligibility.eligible) {
			Logger.info(
				`[Task ${this.taskId}] Execution loop ending for generation ${loopGenerationId}: ${eligibility.reason}`,
			);
			const record =
			...
			if (lastCheckpointMessageIndex !== -1) {
				...
				const commitPromise = (async () => {
					try {
						...
						const errorMessage = error instanceof Error ? error.message : "Unknown error";
						Logger.error(
							`[TaskCheckpointManager] Failed to initialize checkpoint for ${this.taskId}:`,
							errorMessage,
						);
		...
		if (shouldCompact) {
			...
			if (idleGapBeforeLoadContext) {
				return await this.recursivelyMakeDietCodeRequests(
					idleGapBeforeLoadContext,
					includeFileDetails,
					loopGenerationId,
				);
			}
