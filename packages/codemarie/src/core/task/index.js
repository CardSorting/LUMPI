...
import { getGlobalDietCodeRules, getLocalDietCodeRules, refreshDietCodeRulesToggles, } from "@core/context/instructions/user-instructions/dietcode-rules";
...
import { buildJoyRideWorkspaceSnapshot, bumpTaskGeneration, createJoyRideTaskScope, flushTaskGeneration, flushWorkspace, getJoyRideCache, isEnvAlteringCommand, isJoyRideHitDecision, lookupSafeCommandResult, registerTaskLifecycle, storeReusableCommandResult, } from "@core/joyride";
import { parseMentions } from "@core/mentions";
import { CommandPermissionController } from "@core/permissions";
import { summarizeTask } from "@core/prompts/contextManagement";
import { formatResponse } from "@core/prompts/responses";
import { parseSlashCommands } from "@core/slash-commands";
import { ensureRulesDirectoryExists, ensureTaskDirectoryExists, GlobalFileNames, getSavedApiConversationHistory, getSavedDietCodeMessages, } from "@core/storage/disk";
...
import { filterEnabledSkills, filterPromptSkills, getResolvedSkillsForCwd, invalidateSkillsCache, } from "../context/instructions/user-instructions/skills";
...
import { bindTaskLifecycleAuthority, createTaskGenerationId, createTaskLifecycleIntentId, getTaskLifecycleAuthority, taskLifecycleFunnel, } from "./lifecycle/TaskLifecycleFunnel";
import { MessageStateHandler } from "./message-state";
import { StreamResponseHandler } from "./StreamResponseHandler";
import { TaskState } from "./TaskState";
import { ToolExecutor } from "./ToolExecutor";
import { durableGetTaskCompletion, getCachedCompletionFunnelEventFromState, isTaskHarnessTerminal, } from "./tools/completion/CompletionFunnel";
...
import { getToolInvocationContext, } from "./tools/siblings/ToolInvocationContext";
...
import { appendInterruptedAssistantTurn, consumeSteeringInterrupt, shouldAcceptSteeringInterrupt, } from "./utils/steeringInterrupt";
function mapEligibilityToStopReason(eligibility, record) {
    if (!record) {
...
export class Task {
    ...
    idleGapContinuationInProgress = false;
    ...
    latencyTracker;
    siblingBatchSequence = 0;
    activeSiblingScheduler;
    activeSiblingBatchPromise;
    ioAbortController = new AbortController();
    activeSingleIoPromise;
    /** Owned advisory tail; canonical tool settlement never awaits presentation. */
    presentationReplayTail = Promise.resolve();
    constructor(params) {
        const { controller, mcpHub, updateTaskHistory, postStateToWebview, reinitExistingTaskFromId, cancelTask, shellIntegrationTimeout, terminalReuseEnabled, terminalOutputLineLimit, defaultTerminalProfile, cwd, stateManager, workspaceManager, task, images, files, historyItem, taskId, taskLockAcquired, } = params;
        ...
        else if (useMultiRootCheckpointManager &&
            this.taskState.checkpointManagerErrorMessage === multiRootUnsupportedMessage) {
        ...
        const effectiveApiConfiguration = {
            ...
            onRetryAttempt: async (attempt, maxRetries, delay, error) => {
                ...
                if (lastApiReqStartedIndex !== -1) {
                    try {
                        const currentApiReqInfo = JSON.parse(dietcodeMessages[lastApiReqStartedIndex].text || "{}");
        ...
        const commandExecutorCallbacks = {
            ...
            getDietCodeMessages: () => this.messageStateHandler.getDietCodeMessages(),
    ...
    refreshEnvironmentLease() {
        const guard = this.toolExecutor.getGuard();
    ...
    async handleWebviewAskResponse(askResponse, text, images, files) {
        ...
        if (shouldAcceptSteeringInterrupt({
            askResponse,
            feedback,
        ...
        if (shouldAcceptIdleGapFeedback({
            askResponse,
            feedback,
            isStreaming: this.taskState.isStreaming,
            isWaitingForFirstChunk: this.taskState.isWaitingForFirstChunk,
    ...
    async consumeIdleGapFeedbackIfPending() {
    ...
    async queueIdleGapFeedback(feedback) {
    ...
    async rollbackStagedApiUserTurn(apiReqMessageIndex) {
    ...
    async resumeTaskLoopForQueuedIdleGapFeedback() {
    ...
    async continueFromSteeringInterrupt(params, loopGenerationId) {
    ...
    async say(type, text, images, files, partial, auditMetadata, completionFunnelEvent, diagnostics) {
    ...
    async sayAndCreateMissingParamError(toolName, paramName, relPath) {
        await this.say("error", `DietCode tried to use ${toolName}${relPath ? ` for '${relPath.toPosix()}'` : ""} without value for required parameter '${paramName}'. Retrying...`);
    ...
    async switchToPlanModeCallback() {
    ...
    async prepareResumeLifecycle() {
        const authority = this.lifecycleAuthority();
        let record = await authority.restore(this.taskState, this.taskId);
        if (!record) {
            const registered = this.requireLifecycleCommit(await authority.submit(this.taskState, {
        ...
        if (durableCompletion?.status === "succeeded" && record.state !== "terminal") {
            record = this.requireLifecycleCommit(await authority.submit(this.taskState, {
            ...
            }), "Durable completion restoration").record;
        }
        ...
        if (record.state === "suspended") {
            try {
                ...
                const latestRejection = rawDb
                    .prepare("SELECT * FROM task_rejections WHERE taskId = ? AND generationId = ? ORDER BY committedAt DESC LIMIT 1")
                    .get(this.taskId, record.generationId);
                if (latestRejection &&
                    record.lifecycleRevision === latestRejection.lifecycleRevision &&
                    record.cause.reason ===
                        `awaiting_completion_decision:${latestRejection.completionAttemptId}` &&
                    record.cause.originatingOperationId === latestRejection.decisionId) {
                    const activeResult = await authority.submit(this.taskState, {
                        type: "ReactivateAfterCompletionRejection",
                        intentId: createTaskLifecycleIntentId(),
                        taskId: this.taskId,
                        generationId: record.generationId,
                        expectedRevision: latestRejection.lifecycleRevision,
                        completionAttemptId: latestRejection.completionAttemptId,
                        decisionId: latestRejection.decisionId,
                        cause: {
                            source: "recovery",
                            reason: "Reconcile missing reactivation for rejected completion proposal.",
                            originatingOperationId: latestRejection.decisionId,
                        },
                    });
        ...
        if (record.state === "active") {
            ...
            }), "Interrupted task suspension").record;
        }
    ...
    async startTask(task, images, files) {
        ...
        void import("@/services/roadmap/RoadmapLifecycle")
            .then(({ initRoadmapSession }) => initRoadmapSession(this.cwd, this.taskId, { bootstrap: false, forceRefresh: false }))
            .catch((error) => {
    ...
    async resolveResumeAskType(messages) {
        try {
            const durableCompletion = await durableGetTaskCompletion(this.taskId);
    ...
    async resumeTaskFromHistory() {
        ...
        const lastRelevantMessageIndex = findLastIndex(savedDietCodeMessages, (m) => !(m.ask === "resume_task" || m.ask === "resume_completed_task"));
        if (lastRelevantMessageIndex !== -1) {
            savedDietCodeMessages.splice(lastRelevantMessageIndex + 1);
        }
        // since we don't use api_req_finished anymore, we need to check if the last api_req_started has a cost value, if it doesn't and no cancellation reason to present, then we remove it since it indicates an api request without any partial content streamed
        const lastApiReqStartedIndex = findLastIndex(savedDietCodeMessages, (m) => m.type === "say" && m.say === "api_req_started");
        if (lastApiReqStartedIndex !== -1) {
        ...
        try {
            while (true) {
                ...
                if (!eligibility.eligible) {
                    Logger.info(`[Task ${this.taskId}] Task execution loop ending for generation ${loopGenerationId}: ${eligibility.reason}`);
                    break;
                }
                ...
                const loopResult = await this.recursivelyMakeDietCodeRequests(nextUserContent, includeFileDetails, loopGenerationId);
    ...
    async abortTask(options) {
        ...
        if (!lifecycleRecord) {
            ...
            if (activated.kind === "committed") {
                ...
                throw new Error(`Task cancellation could not establish lifecycle authority (${activated.code}): ${activated.reason}`);
            }
        }
        if (lifecycleRecord && lifecycleRecord.state !== "terminal" && lifecycleRecord.cancellation.status === "none") {
            lifecycleRecord = this.requireLifecycleCommit(await lifecycle.submit(this.taskState, {
            ...
            }), "Task cancellation request").record;
        }
        let lifecycleSettlementError;
        try {
            ...
            if (currentLifecycle &&
                ...
                if (settlement.kind === "rejected" && settlement.code !== "terminal_generation") {
                    lifecycleSettlementError = new Error(`Lifecycle cancellation settlement failed (${settlement.code}): ${settlement.reason}`);
                }
            }
        ...
        if (lifecycleSettlementError)
            throw lifecycleSettlementError;
    }
    // Tools
    async executeCommandTool(command, timeoutSeconds, options) {
        ...
        if (isEnvAlteringCommand(command) && !result[0]) {
            Logger.info(`[Task ${this.taskId}] Env-altering command detected (\`${command}\`). Revoking environmental lease for re-probe.`);
     ...
     */
    async cancelBackgroundCommand(ownerId) {
    ...
    isHarnessTerminalForNoToolsNudge() {
        return isTaskHarnessTerminal(this.taskState);
    }
    buildNoToolsUsedNudge() {
     ...
     */
    async applySilentTurnBoundaryContextRollover(totalTokens) {
        const apiConversationHistory = this.messageStateHandler.getApiConversationHistory();
        const { contextWindow, maxAllowedSize } = getContextWindowInfo(this.api);
        const keep = totalTokens / 2 > maxAllowedSize ? "quarter" : "half";
        const newDeletedRange = this.contextManager.getNextTruncationRange(apiConversationHistory, this.taskState.conversationHistoryDeletedRange, keep);
        ...
        this.taskState.conversationHistoryDeletedRange = newDeletedRange;
        await this.messageStateHandler.saveDietCodeMessagesAndUpdateHistory();
        await this.contextManager.triggerApplyStandardContextTruncationNoticeChange(Date.now(), await ensureTaskDirectoryExists(this.taskId), apiConversationHistory, true);
        Logger.info(`[Task ${this.taskId}] Applied transparent recoverable context rollover through message ${newDeletedRange[1]}`);
    ...
    async *attemptApiRequest(previousApiReqIndex) {
        ...
        const globalRules = await getGlobalDietCodeRules(globalDietCodeRulesFilePath, globalToggles, {
            evaluationContext,
        });
        ...
        let dietcodeIgnoreInstructions;
        ...
        const promptContext = {
            ...
            dietcodeWebToolsEnabled: this.stateManager.getGlobalSettingsKey("dietcodeWebToolsEnabled") &&
        ...
        const contextManagementMetadata = await this.contextManager.getNewContextMessagesAndMetadata(this.messageStateHandler.getApiConversationHistory(), this.messageStateHandler.getDietCodeMessages(), this.api, this.taskState.conversationHistoryDeletedRange, previousApiReqIndex, await ensureTaskDirectoryExists(this.taskId), this.stateManager.getGlobalSettingsKey("useAutoCondense"));
                    ...
                    if (truncatedRange.length > 0) {
                        const snapshotContent = truncatedRange
                            ...
                            const content = Array.isArray(m.content)
                                ? m.content
                                    .map((b) => "text" in b ? b.text : b.type === "tool_use" ? `[Tool Use: ${b.name}]` : "")
                                    .join("\n")
                                : m.content;
        ...
        const requestSystemPrompt = this.contextManager.getSystemPromptForProjection(systemPrompt, contextManagementMetadata.truncatedConversationHistory);
        ...
        try {
            ...
            if (isContextWindowExceededError && !this.taskState.didAutomaticallyRetryFailedApiRequest) {
                ...
                const lastApiReqStartedIndex = findLastIndex(this.messageStateHandler.getDietCodeMessages(), (m) => m.say === "api_req_started");
                if (lastApiReqStartedIndex !== -1) {
                    const dietcodeMessages = this.messageStateHandler.getDietCodeMessages();
                    const currentApiReqInfo = JSON.parse(dietcodeMessages[lastApiReqStartedIndex].text || "{}");
                ...
                if (!isDietCodeProviderInsufficientCredits && !isAuthError && this.taskState.autoRetryAttempts < 3) {
                    ...
                    const autoRetryApiReqIndex = findLastIndex(this.messageStateHandler.getDietCodeMessages(), (m) => m.say === "api_req_started");
                    if (autoRetryApiReqIndex !== -1) {
                        const dietcodeMessages = this.messageStateHandler.getDietCodeMessages();
                        const currentApiReqInfo = JSON.parse(dietcodeMessages[autoRetryApiReqIndex].text || "{}");
                        delete currentApiReqInfo.streamingFailedMessage;
                ...
                const manualRetryApiReqIndex = findLastIndex(this.messageStateHandler.getDietCodeMessages(), (m) => m.say === "api_req_started");
                if (manualRetryApiReqIndex !== -1) {
                    const dietcodeMessages = this.messageStateHandler.getDietCodeMessages();
                    const currentApiReqInfo = JSON.parse(dietcodeMessages[manualRetryApiReqIndex].text || "{}");
                    delete currentApiReqInfo.streamingFailedMessage;
    ...
    async resolveSiblingWorkspaceLocality(blocks) {
        ...
        const workers = Array.from({ length: Math.min(DEFAULT_SIBLING_TOOL_CONCURRENCY, blocks.length) }, async () => {
            while (cursor < blocks.length) {
                ...
                if (!isIoAuthorityTool(block.name) || !(block.params.path ?? block.params.absolutePath)?.trim())
                    continue;
                try {
    ...
    async replaySiblingPresentation(events) {
    ...
    scheduleSiblingPresentation(batchId, progress, batches) {
        this.presentationReplayTail = Promise.all([this.presentationReplayTail, progress])
            ...
            for (const events of batches)
                await this.replaySiblingPresentation(events);
    ...
    async executeSiblingToolBatch(blocks) {
        ...
        const scheduler = new SiblingToolScheduler({
            ...
            classifyResult: (value) => {
                ...
                if (!event)
                    return { status: value.outcome ?? "succeeded", error: value.error };
                return {
            ...
            run: async (node, signal) => {
                if (signal.aborted)
                    throw new Error("Task cancelled before sibling execution");
                return await this.toolExecutor.executeToolCaptured(node.block, node.sequence, node.capturePresentation, node.id, signal);
            },
        });
        ...
        let releaseBatchJoin;
        const batchJoin = new Promise((resolve) => {
        ...
        try {
            ...
            if (this.activeSiblingScheduler === scheduler)
                this.activeSiblingScheduler = undefined;
            releaseBatchJoin();
            if (this.activeSiblingBatchPromise === batchJoin)
                this.activeSiblingBatchPromise = undefined;
        }
        ...
        for (const envelope of envelopes) {
            ...
            if (!resultContent) {
                resultContent = await this.toolExecutor.captureSyntheticToolResult(envelope.node.block, envelope.sequence, envelope.id, formatResponse.toolError(envelope.error ?? `Sibling ${envelope.status}`));
            }
        ...
        this.scheduleSiblingPresentation(batchId, progressPromise, envelopes.map((envelope) => envelope.value?.presentationEvents ?? []));
    }
    async presentAssistantMessage() {
        ...
        switch (block.type) {
            ...
            case "tool_use": {
                const completeSiblings = [];
                if (this.isParallelToolCallingEnabled()) {
                    for (const candidate of this.taskState.assistantMessageContent.slice(this.taskState.currentStreamingContentIndex)) {
                        if (candidate.type !== "tool_use" || candidate.partial)
                            break;
                ...
                const isScratchpadReadMutation = block.name === DietCodeDefaultTool.FILE_READ &&
                    path.basename(block.params.path?.trim() ?? "").toLowerCase() === "scratchpad.md";
                const isReadOnlyTool = (READ_ONLY_TOOLS.includes(block.name) && !isScratchpadReadMutation) ||
                ...
                if (isIoAuthorityTool(block.name))
                    this.activeSingleIoPromise = execution;
                try {
                    await execution;
                }
                finally {
                    if (this.activeSingleIoPromise === execution)
                        this.activeSingleIoPromise = undefined;
                }
                if (!block.partial) {
                    this.latencyTracker.markIoStage("projection_ready", {
    ...
    async recursivelyMakeDietCodeRequests(userContent, includeFileDetails = false, loopGenerationId = this.taskState.executionGeneration) {
        ...
        if (!eligibility.eligible) {
            ...
            const record = lifecycleAuthority.readProjection(this.taskState) ??
        ...
        if (this.taskState.consecutiveMistakeCount >= this.stateManager.getGlobalSettingsKey("maxConsecutiveMistakes") &&
            ...
            try {
                const askResult = await this.ask("mistake_limit_reached", `I have made ${this.taskState.consecutiveMistakeCount} consecutive mistakes (e.g. repeated errors or no tools used). Please help guide my next steps.`);
        ...
        const previousApiReqIndex = findLastIndex(this.messageStateHandler.getDietCodeMessages(), (m) => m.say === "api_req_started");
        // Save checkpoint if this is the first API request
        const isFirstRequest = this.messageStateHandler.getDietCodeMessages().filter((m) => m.say === "api_req_started").length === 0;
        ...
        if (isFirstRequest &&
            ...
            const lastCheckpointMessageIndex = findLastIndex(this.messageStateHandler.getDietCodeMessages(), (m) => m.say === "checkpoint_created");
        ...
        if (useAutoCondense) {
            ...
            if (this.taskState.currentlySummarizing) {
                ...
                shouldCompact = this.contextManager.shouldCompactContextWindow(this.messageStateHandler.getDietCodeMessages(), this.api, previousApiReqIndex);
                ...
                if (shouldCompact) {
                    shouldCompact = await this.contextManager.attemptFileReadOptimization(this.messageStateHandler.getApiConversationHistory(), this.taskState.conversationHistoryDeletedRange, this.messageStateHandler.getDietCodeMessages(), previousApiReqIndex, await ensureTaskDirectoryExists(this.taskId), this.api);
                }
                ...
                if (shouldCompact) {
                    const telemetry = this.contextManager.getContextTelemetryData(this.messageStateHandler.getDietCodeMessages(), this.api, previousApiReqIndex);
        ...
        if (shouldCompact) {
            ...
            [parsedUserContent, environmentDetails, dietcoderulesError] = await this.loadContext(userContent, includeFileDetails, useCompactPrompt);
        }
        ...
        if (dietcoderulesError === true) {
            await this.say("error", "Issue with processing the /newrule command. Double check that, if '.dietcoderules' already exists, it's a directory and not a file. Otherwise there was an issue referencing this file/directory.");
        }
        ...
        try {
            ...
            const finalizePendingReasoningMessage = async (thinking) => {
                const pendingReasoningIndex = findLastIndex(this.messageStateHandler.getDietCodeMessages(), (message) => message.type === "say" && message.say === "reasoning" && message.partial === true);
                ...
                for await (const chunk of stream) {
                    ...
                    switch (chunk.type) {
                        ...
                        case "text": {
                            ...
                            const recognizedTool = this.taskState.assistantMessageContent.find((candidate) => candidate.type === "tool_use");
            ...
            let earlyNativePresentation;
            const finalizedNativeToolBlocks = toolUseHandler.getPartialToolUsesAsContent()?.map((block) => ({ ...block, partial: false })) ?? [];
            if (this.useNativeToolCalls &&
                ...
                const allEarlyQueries = finalizedNativeToolBlocks.every((block) => isIoAuthorityTool(block.name) &&
                    (!block.params.path?.trim() ||
                        isLocatedInPath(this.cwd, path.resolve(this.cwd, block.params.path))) &&
            ...
            try {
                ...
                await EnvironmentTracker.recordUsage(this.cwd, "cline-agent", {
            ...
            if (assistantHasContent) {
                ...
                const recDidEndLoop = await this.recursivelyMakeDietCodeRequests(this.taskState.userMessageContent, false, loopGenerationId);
                ...
                const baseErrorMessage = "Invalid API Response: The provider returned an empty or unparsable response. This is a provider-side issue where the model failed to generate valid output or returned tool calls that DietCode cannot process. Retrying the request may help resolve this issue.";
    ...
    async loadContext(userContent, includeFileDetails = false, useCompactPrompt = false) {
        ...
        const parseTextBlock = async (text) => {
            ...
            const { processedText, needsDietCoderulesFileCheck: needsCheck } = await parseSlashCommands(parsedText, localWorkflowToggles, globalWorkflowToggles, ulid, focusChainSettings, useNativeToolCalls, providerInfo, mcpPromptFetcher, cwd, this.stateManager.getGlobalSettingsKey("modEnabled") ?? false);
            if (needsCheck) {
    ...
    async getKnowledgeGraphService() {
        ...
        const provider = apiConfiguration.embeddingProvider;
        ...
        else if (this.api && typeof this.api.embedText === "function") {
    ...
    async loadWorkspaceIntelligence() {
        try {
