...
import { type BlobStoreSettings, blobStorage } from "../../storage/DietCodeBlobStorage";
...
export function getBlobStoreSettingsFromEnv(): BlobStoreSettings {
	return {
		...
		secretAccessKey:
			process?.env?.DIETCODE_STORAGE_SECRET_ACCESS_KEY || process?.env?.CLINE_STORAGE_SECRET_ACCESS_KEY || "",
		...
		maxRetries: parseIntEnv(
			process.env.DIETCODE_STORAGE_SYNC_MAX_RETRIES || process.env.CLINE_STORAGE_SYNC_MAX_RETRIES,
			5,
		),
		batchSize: parseIntEnv(
			process.env.DIETCODE_STORAGE_SYNC_BATCH_SIZE || process.env.CLINE_STORAGE_SYNC_BATCH_SIZE,
			10,
		),
		...
		backfillEnabled:
			(process.env.DIETCODE_STORAGE_SYNC_BACKFILL_ENABLED || process.env.CLINE_STORAGE_SYNC_BACKFILL_ENABLED) ===
			"true",
	};
}
...
type ResolvedSyncWorkerOptions = SyncWorkerOptions &
	Required<
		Pick<
			BlobStoreSettings,
			"intervalMs" | "maxRetries" | "batchSize" | "maxQueueSize" | "maxFailedAgeMs" | "backfillEnabled"
		>
	>;
