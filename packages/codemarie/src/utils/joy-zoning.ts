...
export function isLayerTagSupported(filePath: string, content?: string): boolean {
	...
	const isExtSupported =
		supportedExtensions && supportedExtensions.length > 0 ? supportedExtensions.includes(ext) : !!style;
...
export function suggestLayerForContent(content: string): { layer: Layer; reason: string } | null {
	...
	if (
		/EventEmitter|Observable|Subject|BehaviorSubject|ReplaySubject|Subscription|Redux|Store|Dispatch|Effect/i.test(
			content,
		) ||
	...
	if (
		...
		if (!/import\s+.*from\s+["'](?:fs|http|pg|mysql|redis|axios|sqlite|mongodb)/i.test(content)) {
			return {
				layer: "domain",
				reason:
					"Contains Domain-Driven Design (DDD) patterns (ValueObject, Entity, AggregateRoot) — belongs in the Domain layer.",
			};
		}
	}
