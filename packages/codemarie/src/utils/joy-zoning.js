...
let SPEC_CACHE = null;
let EXCLUDE_PATTERN_CACHE = null;
const PATH_LAYER_CACHE = new Map();
const PATH_TAG_SUPPORT_CACHE = new Map();
/**
 ...
 */
export function getLayer(filePath, content) {
    ...
    if (PATH_LAYER_CACHE.has(normalized))
        return PATH_LAYER_CACHE.get(normalized);
    ...
    const layer = normalized.includes("src/domain/") || normalized.endsWith("/src/domain")
        ? "domain"
 ...
 */
export function isLayerTagSupported(filePath, content) {
    const normalized = filePath.replace(/\\/g, "/");
    if (!content && PATH_TAG_SUPPORT_CACHE.has(normalized))
        return PATH_TAG_SUPPORT_CACHE.get(normalized);
    const ext = path.extname(filePath).toLowerCase();
    if (filePath.toLowerCase().endsWith(".d.ts") || STRICT_BLOCKLIST.includes(ext))
        return false;
    ...
    if (EXCLUDE_PATTERN_CACHE === null) {
        EXCLUDE_PATTERN_CACHE = excludePaths.map((p) => (f) => minimatch(f, p, { dot: true }));
    }
    ...
    if (!content)
        PATH_TAG_SUPPORT_CACHE.set(normalized, true);
    return true;
}
/**
 ...
 */
export function generateLayerComment(filePath, layer, content) {
 ...
 */
export function validateSmells(filePath, content) {
    const errors = [];
    ...
    if (layer === "domain") {
        ...
        if (classCount > 3 || (classCount > 1 && totalLines > 500)) {
            errors.push(`${path.basename(filePath)}: Multiple classes in a single file — consider splitting for better domain isolation.`);
        }
    }
 ...
 */
export function validateLayering(filePath, content) {
    const errors = [];
    const layer = getLayer(filePath);
 ...
 */
export function parseLayerTag(content) {
 ...
 */
export function validateImportDepth(filePath, content) {
    const errors = [];
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (line.includes('from "') || line.includes("from '")) {
            ...
            if (match) {
                ...
                if (depth > 3) {
                    errors.push(`${path.basename(filePath)}:${i + 1}: Excessive relative navigation (${depth} levels) — use @ aliases or flatten structure.`);
                }
            }
        }
    }
 ...
 */
export function validateJoyZoning(filePath, content) {
    const allErrors = [];
    ...
    if (!tag) {
        ...
        allErrors.push(`${path.basename(filePath)}: Geographic Misalignment — Tag [LAYER: ${tag.toUpperCase()}] does not match path layer '${pathLayer}'.`);
    }
    // 2. Import Depth
 ...
 */
export function suggestLayerForContent(content) {
    // 1. UI Patterns
    ...
    if (/import\s+.*from\s+["'](?:fs|http|https|net|child_process|pg|mysql|redis|axios|sqlite|mongodb)/i.test(content) ||
    ...
    if (/EventEmitter|Observable|Subject|BehaviorSubject|ReplaySubject|Subscription|Redux|Store|Dispatch|Effect/i.test(content) ||
    ...
    if (/ValueObject|Entity|AggregateRoot|Specification|DomainEvent/i.test(content) ||
 ...
 */
export function getTargetPath(params) {
    if (!params)
        return null;
...
//# sourceMappingURL=joy-zoning.js.map