 ...
 */
export function sanitizeCellOutputs(cell) {
    if (cell.cell_type !== "code" || !cell.outputs || !Array.isArray(cell.outputs)) {
        return cell;
    }
    const sanitizedOutputs = cell.outputs.map((output) => {
 ...
 */
export function sanitizeNotebookForLLM(jsonString, stripAllOutputs = false) {
    try {
        ...
        if (stripAllOutputs) {
            ...
            notebook.cells = notebook.cells.map((cell) => sanitizeCellOutputs(cell));
        }
 ...
 */
export function sanitizeCellForLLM(cell) {
