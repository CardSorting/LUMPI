...
export class TaskCheckpointManager implements ICheckpointManager {
	...
	async saveCheckpoint(isAttemptCompletionMessage = false, completionMessageTs?: number): Promise<void> {
		try {
			...
			if (
				...
				!this.state.checkpointManagerErrorMessage
			) {
	...
	async restoreCheckpoint(
		...
		try {
			...
			if (!message) {
				Logger.error(
					`[TaskCheckpointManager] Message not found for timestamp ${messageTs} in task ${this.task.taskId}`,
				);
				return {};
			}
			...
			if (!didWorkspaceRestoreFail) {
				...
				if (this.state.conversationHistoryDeletedRange !== undefined) {
					checkpointManagerStateUpdate.conversationHistoryDeletedRange =
	...
	async presentMultifileDiff(messageTs: number, seeNewChangesSinceLastTaskCompletion: boolean): Promise<void> {
		...
		try {
			...
			Logger.log(
				`[TaskCheckpointManager] presentMultifileDiff for task ${this.task.taskId}, messageTs: ${messageTs}`,
			);
			const dietcodeMessages = this.services.messageStateHandler.getDietCodeMessages();
	...
	async commit(): Promise<string | undefined> {
		try {
			...
			if (!this.state.checkpointTracker) {
				Logger.error(
					`[TaskCheckpointManager] Checkpoint tracker not available for commit in task ${this.task.taskId}`,
				);
				return undefined;
			}
	...
	async createGhostBranch(branchName: string, baseRef?: string): Promise<void> {
		...
		if (this.state.checkpointTracker) {
			...
			if (kgService) {
				await kgService.addKnowledge(
					this.task.taskId,
					"snapshot:ghost_branch",
					`Ghost Branch Created: ${branchName}`,
					{
	...
	private async getWorkspacePath(): Promise<string> {
		...
		if (this.services.workspaceManager) {
			try {
				...
				Logger.warn(
					`[TaskCheckpointManager] WorkspaceRootManager returned no primary root for task ${this.task.taskId}`,
				);
			} catch (error) {
				Logger.warn(
