...
export class SQLiteMaintenanceEngine {
        ...
        try {
            ...
            try {
                await db
                    ...
                    .where("expiresAt", "<", now)
                    .executeTakeFirst();
                await db.executeQuery(CompiledQuery.raw(`DELETE FROM swarm_lock_generations WHERE resourceKey NOT IN (SELECT resource FROM swarm_locks)`));
            }
            catch { }
            ...
            try {
                ...
                if (currentTelemetryCount > telemetryMaxRows) {
                    ...
                    await db.executeQuery(CompiledQuery.raw(`DELETE FROM telemetry WHERE id IN (SELECT id FROM telemetry ORDER BY timestamp ASC LIMIT ${deleteCount})`));
            ...
            try {
                ...
                if (currentAuditCount > auditMaxRows) {
                    ...
                    await db.executeQuery(CompiledQuery.raw(`DELETE FROM audit_events WHERE id IN (SELECT id FROM audit_events ORDER BY createdAt ASC LIMIT ${deleteCount})`));
            ...
            try {
                ...
                const edgeRes1 = await db.executeQuery(CompiledQuery.raw(`DELETE FROM knowledge_edges WHERE sourceId NOT IN (SELECT id FROM knowledge) OR targetId NOT IN (SELECT id FROM knowledge)`));
                prunedOrphanEdges += Number(edgeRes1.numAffectedRows || 0);
                // Delete orphaned agent_knowledge_edges
                const edgeRes2 = await db.executeQuery(CompiledQuery.raw(`DELETE FROM agent_knowledge_edges WHERE sourceId NOT IN (SELECT id FROM agent_knowledge) OR targetId NOT IN (SELECT id FROM agent_knowledge)`));
            ...
            try {
                ...
                if (currentSnapshotsCount > snapshotsMaxRows) {
                    ...
                    await db.executeQuery(CompiledQuery.raw(`DELETE FROM agent_cognitive_snapshots WHERE id IN (SELECT id FROM agent_cognitive_snapshots ORDER BY createdAt ASC LIMIT ${deleteCount})`));
                }
                // Prune orphaned telemetry_aggregates for task_* IDs no longer in tasks/agent_tasks
                await db.executeQuery(CompiledQuery.raw(`DELETE FROM telemetry_aggregates WHERE id LIKE 'task_%' AND SUBSTR(id, 6) NOT IN (SELECT id FROM agent_tasks)`));
            }
            catch { }
            ...
            try {
                ...
                if (currentNodesCount > nodesMaxRows) {
                    ...
                    const delRes = await db.executeQuery(CompiledQuery.raw(`DELETE FROM nodes WHERE id IN (SELECT id FROM nodes ORDER BY timestamp ASC LIMIT ${deleteCount})`));
            ...
            try {
                ...
                if (currentTreesCount > treesMaxRows) {
                    ...
                    const delRes = await db.executeQuery(CompiledQuery.raw(`DELETE FROM trees WHERE id IN (SELECT id FROM trees ORDER BY createdAt ASC LIMIT ${deleteCount})`));
            ...
            try {
                ...
                await db.executeQuery(CompiledQuery.raw(`DELETE FROM agent_memory WHERE streamId NOT IN (SELECT id FROM agent_streams)`));
            }
            catch { }
            try {
            ...
            try {
                ...
                const delFilesRes = await db.executeQuery(CompiledQuery.raw(`DELETE FROM files WHERE id NOT IN (SELECT id FROM trees) AND NOT EXISTS (SELECT 1 FROM trees WHERE INSTR(trees.entries, files.id) > 0)`));
            ...
            try {
                let freelist = rawDb.pragma("freelist_count", { simple: true }) || 0;
                let passes = 0;
                ...
                while (freelist > 0 && passes < MAX_VACUUM_PASSES) {
                    ...
                    freelist = rawDb.pragma("freelist_count", { simple: true }) || 0;
            ...
            try {
                ...
                if (res?.[0]?.busy) {
                    ...
                    res = rawDb.pragma("wal_checkpoint(PASSIVE)");
                }
                if (res?.[0]) {
                    walResult = {
                        busy: res[0].busy ?? 0,
    ...
    async getStorageHealthReport() {
        ...
        if (freelistRatio > 0.3) {
            ...
            recommendations.push(`Freelist page ratio is high (${(freelistRatio * 100).toFixed(1)}%). Incremental vacuum recommended.`);
        }
        if (metrics.walSizeBytes > 32 * 1024 * 1024) {
            healthStatus = "critical";
            recommendations.push(`WAL log file size is large (${(metrics.walSizeBytes / 1024 / 1024).toFixed(1)} MB). Forced WAL checkpoint recommended.`);
        }
        return {
...
//# sourceMappingURL=SQLiteMaintenanceEngine.js.map