...
function cacheKey(workspace) {
    return path.resolve(workspace);
}
export function invalidateSessionBriefCache(workspace) {
    if (!workspace) {
        briefCache.clear();
        return;
    }
...
export async function sessionBrief(workspace, forceRefresh = false) {
    ...
    try {
        ...
        const hints = (status._roadmap_operator_hints || {});
        const brief = {
            enabled: true,
            success: true,
            workspace,
            ...
            last_validated_at: status.workspace_state?.last_validated_at,
            last_mutated_at: status.workspace_state?.last_mutated_at,
...
export function formatRoadmapEnvironmentSection(brief) {
    return formatRoadmapSteeringBlock(brief);
}
export async function getRoadmapEnvironmentSection(workspace) {
    const cfg = getRoadmapConfig();
    if (!cfg.enabled) {
        return "";
    }
...
//# sourceMappingURL=RoadmapSession.js.map