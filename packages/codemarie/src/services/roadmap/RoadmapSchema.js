...
export function findBootstrapPlaceholders(content) {
    const issues = [];
...
export function getSectionBody(content, sectionTitle) {
...
function countSubsections(sectionBody) {
    const matches = sectionBody.match(/^###\s+\d+\.\s+/gm);
    return matches ? matches.length : 0;
}
export function validateRoadmapContent(content) {
    const issues = [];
...
export function bootstrapSkeleton(params) {
    ...
    const workflows = params.canonical_workflows?.trim() ||
        "Preserve primary agent and operator flows identified in README and recent commits.";
    const runtime = params.runtime_center?.trim() ||
    ...
    const drift_lines = params.changed_files && params.changed_files.length > 0
    ...
    const next_move = params.checkpoint_next_move?.trim() ||
        "Populate Now with 1–3 evidence-backed items connected to center of gravity.";
    return `# ROADMAP.md
...
//# sourceMappingURL=RoadmapSchema.js.map