...
export function formatCockpitReport(payload, options) {
    ...
    if (payload.health_status)
        lines.push(`Health: ${payload.health_status}`);
    if (payload.phase)
        lines.push(`Phase: ${payload.phase}`);
    if (payload.recent_checkpoint_date)
        lines.push(`Last checkpoint: ${payload.recent_checkpoint_date}`);
    if (payload.code_soup_risk)
        lines.push(`Code soup risk: ${payload.code_soup_risk}`);
    if (payload.now_item_count !== undefined)
        lines.push(`Now items: ${payload.now_item_count}`);
    ...
    const tv = (payload.temporal_validity || payload.checkpoint_freshness?.temporal_validity);
    if (tv) {
    ...
    if (payload.validation_pending)
        lines.push(`⚠️ validation_pending — ${AUTO_GOVERNANCE.validationAtCompletion}`);
    if (payload.bootstrap_complete === false) {
        lines.push(`⚠️ bootstrap incomplete (${payload.bootstrap_placeholder_count ?? "?"} phrases)`);
    }
    const gate = (payload.roadmap_gate || {});
    ...
    const gateLine = formatKanbanGateStatusLine({
        ...
        schemaValid: payload.schema_valid,
    ...
    const verify = payload.project_steering_digest?.verification_commands || [];
    if (verify.length > 0)
        lines.push(`Verify: ${verify[0]}`);
    const runtimeState = (payload.runtime_state ||
        payload.workspace_state?.runtime_state);
    if (runtimeState) {
    ...
    let lineage = payload.workspace_state?.lineage;
    if (lineage && lineage.length > 0) {
        if (agentId && !verbose) {
            lineage = lineage.filter((entry) => !entry.agent_id || entry.agent_id === agentId);
        }
        if (lineage.length > 0) {
    ...
    lines.push("", `→ ${payload.recommended_next_action?.command || payload.agent_next_call || "roadmap(action='guide')"}`);
    return lines.join("\n");
}
export async function buildCockpitPayload(roadmapService, workspace, options) {
...
//# sourceMappingURL=RoadmapCockpit.js.map