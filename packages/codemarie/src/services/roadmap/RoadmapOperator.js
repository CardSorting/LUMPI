...
import { AUTO_GOVERNANCE, formatKanbanGateStatusLine, gateEditInstruction, isAutoClearableGovernanceOnly, ROADMAP_DIAGNOSTIC_SLASH_COMMANDS, } from "./RoadmapAutoGovernance.js";
...
export function isBootstrapIncomplete(params) {
    ...
    const complete = params.bootstrap_complete !== undefined && params.bootstrap_complete !== null
        ? params.bootstrap_complete
        : ws.bootstrap_complete;
    const count = params.bootstrap_placeholder_count !== undefined && params.bootstrap_placeholder_count !== null
        ? params.bootstrap_placeholder_count
        : ws.bootstrap_placeholder_count;
    if (complete === false)
        return true;
    if (complete === true)
        return false;
...
export function determinePhase(params) {
    ...
    if (params.bootstrap_incomplete) {
        return {
            ...
            operator_summary: "Bootstrap template phrases remain — autofill runs at attempt_completion; preview with roadmap(action='apply_bootstrap_fill').",
...
export function roadmapToolCommandToSlash(command) {
...
export function gateExplainParamsFromStatus(workspace, gate, status) {
    return {
        workspace,
...
export function formatExplainGateReport(params) {
    ...
    if (closed.length > 0 || params.open_gates) {
        ...
        lines.push("");
        const blocking = (params.blocking_gates || params.closed_gates || []);
    ...
    if (params.freshness) {
        ...
        if (params.freshness.summary)
            lines.push(String(params.freshness.summary));
    }
    return lines.join("\n");
}
export function buildAgentOperatorHints(params) {
    const snap = params.gate || {};
    const wsState = (snap.workspace_state || {});
    ...
    const nextRec = params.recommended_next_action ||
        recommendNextAction({
...
export function wrapClarityEnvelope(payload, phaseInfo) {
    const gate = (payload.roadmap_gate || null);
    const digest = (payload.project_steering_digest || {});
    const operatorHints = buildAgentOperatorHints({
        ...
        bootstrap_fill_hint: payload.bootstrap_fill_plan
            ? `${payload.bootstrap_fill_plan.remaining_count || 0} bootstrap phrase(s) — autofill runs at attempt_completion`
            : undefined,
    });
    const identity = payload.project_identity_line ||
        operatorHints.project_identity_line ||
        digest.identity_line ||
        payload.steering_brief;
    const includePlaybook = ["guide", "doctor"].includes(String(payload.action || "")
        .trim()
        .toLowerCase());
    return {
        ...payload,
        ...
        governance_policy: AUTO_GOVERNANCE.governancePolicy,
        auto_clearable_governance_only: payload.auto_clearable_governance_only ?? operatorHints.auto_clearable_governance_only ?? false,
        ...
        _roadmap_operator_hints: {
            ...
            next_action: operatorHints.next_action ||
...
//# sourceMappingURL=RoadmapOperator.js.map