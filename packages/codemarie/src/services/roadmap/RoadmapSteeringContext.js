...
export async function buildSteeringContext(workspace) {
    ...
    const fp = (status.project_fingerprint || {});
    const digest = (status.project_steering_digest || {});
...
export function mergeSteeringFields(payload, steering) {
    const out = { ...payload };
    if (steering.workspace && !out.workspace)
    ...
    if (steering.roadmap_path && !out.roadmap_path)
    ...
    if (steering.phase && !out.phase)
        out.phase = steering.phase;
    return out;
}
export async function enrichPayloadWithSteering(payload) {
...
//# sourceMappingURL=RoadmapSteeringContext.js.map