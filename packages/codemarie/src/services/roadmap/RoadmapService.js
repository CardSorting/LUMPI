...
import { determinePhase, formatExplainGateReport, gateExplainParamsFromStatus, isBootstrapIncomplete, wrapClarityEnvelope as operatorWrapClarityEnvelope, recommendNextAction, } from "./RoadmapOperator.js";
import { clearLastError, formatWatchReport, readCurrentProgress, readLastError, recordLastError, } from "./RoadmapProgress.js";
import { bootstrapSkeleton, findBootstrapPlaceholders, getSectionBody, HEALTH_STATUSES, REQUIRED_SECTIONS, SOUP_RISK_LEVELS, validateRoadmapContent, } from "./RoadmapSchema.js";
import { BUNDLED_SKILL_REL } from "./RoadmapSkillInstall.js";
import { buildSnapshotKey, getSnapshotFromCache, setSnapshotCache } from "./RoadmapSnapshot.js";
...
async function readText(filePath, limit = 8000) {
    try {
...
export function hydrateRuntimeState(content) {
...
export function projectRuntimeStateToMarkdown(state) {
    let md = `# ROADMAP.md\n\n`;
...
async function writeRoadmapAtomically(workspace, content) {
...
export async function computeDependencyManifestsHash(workspace) {
...
export function slimEvidence(evidence) {
    if (!evidence)
        return evidence;
    return {
        ...
        readmes: (evidence.readmes || []).map((r) => ({
            path: r.path,
            excerpt_length: r.excerpt ? r.excerpt.length : 0,
        })),
        architecture_docs: (evidence.architecture_docs || []).map((d) => ({
            path: d.path,
            excerpt_length: d.excerpt ? d.excerpt.length : 0,
        })),
        configs: (evidence.configs || []).map((c) => ({
            path: c.path,
        ...
        todo_markers: (evidence.todo_markers || []).map((t) => ({ file: t.file, line: t.line, marker: t.marker })),
...
async function recordMutationLineage(workspace, entry) {
    const roadmapPath = path.join(workspace, "ROADMAP.md");
    let fileHash = "";
        ...
        for (const entry of entries) {
            ...
            if (entry.isDirectory()) {
                ...
                if (testFileCount < TEST_LIMIT) {
                    if ((nameLower.startsWith("test_") && suffix === ".py") ||
                ...
                if (needTodo && size > TODO_MAX_BYTES)
                    needTodo = false;
                if (needSource && size > SOURCE_MAX_BYTES)
                    needSource = false;
                if (!needTodo && !needSource) {
                    continue;
                }
...
async function getGitRemoteSummary(workspace) {
...
async function getGitRecentChanges(workspace, light = false) {
    if (!(await isDir(path.join(workspace, ".git")))) {
        return {
            available: false,
            recent_commits: [],
...
async function getPackageDescription(root) {
    const pkgPath = path.join(root, "package.json");
    if (await fileExists(pkgPath)) {
        try {
            const data = JSON.parse(await readText(pkgPath));
            if (data && data.description)
                return String(data.description).trim().slice(0, 400);
        }
        catch { }
    }
    const pyproject = path.join(root, "pyproject.toml");
...
async function getReadmeTitle(root) {
...
async function getReadmeTagline(root) {
    for (const name of ["README.md", "readme.md", "docs/README.md"]) {
        const filePath = path.join(root, name);
        if (!(await fileExists(filePath)))
            continue;
...
async function getPackageScripts(root) {
    const scripts = [];
    const pkgPath = path.join(root, "package.json");
    if (await fileExists(pkgPath)) {
        try {
...
async function getLicenseLabel(root) {
    const pkgPath = path.join(root, "package.json");
    if (await fileExists(pkgPath)) {
        try {
            const data = JSON.parse(await readText(pkgPath));
            if (data && data.license)
                return String(data.license).slice(0, 80);
        }
        catch { }
    }
...
async function getDocsRoots(root) {
    const found = [];
...
async function detectMarkersAsync(root, markers, dirOk = false) {
    const found = [];
...
async function getRuntimeVersions(root) {
    const versions = {};
    const candidates = [
        [".nvmrc", "node"],
        [".node-version", "node"],
...
async function getComposeServices(root) {
...
async function getCIWorkflowNames(root) {
    const wfDir = path.join(root, ".github", "workflows");
    if (!(await isDir(wfDir)))
        return [];
    try {
...
async function getIssueTemplates(root) {
    const found = [];
...
async function getMakefileTargets(root) {
...
export async function buildProjectFingerprint(workspace) {
    ...
    const has_docker = (await fileExists(path.join(workspace, "Dockerfile"))) ||
        (await fileExists(path.join(workspace, "docker-compose.yml")));
    const has_tests = test_frameworks.length > 0 ||
    ...
    if (await fileExists(path.join(workspace, "plugin.yaml"))) {
        ...
        if ((await isDir(path.join(workspace, "app"))) ||
    ...
    const dependency_automation = [];
    if ((await fileExists(path.join(workspace, "renovate.json"))) ||
        (await fileExists(path.join(workspace, ".github", "renovate.json")))) {
        dependency_automation.push("Renovate");
    }
    if ((await fileExists(path.join(workspace, ".github", "dependabot.yml"))) ||
        (await fileExists(path.join(workspace, ".github", "dependabot.yaml")))) {
        dependency_automation.push("Dependabot");
    }
    const has_codeowners = (await fileExists(path.join(workspace, ".github", "CODEOWNERS"))) ||
    ...
    const stack_parts = [];
    if (primary_language)
        stack_parts.push(primary_language);
    for (const f of frameworks) {
        if (!stack_parts.includes(f))
            stack_parts.push(f);
    }
...
async function assessCodeSoup(workspace, heavy) {
...
function countSectionItems(content, sectionTitle) {
...
function parseRoadmapText(content, pathStr) {
    ...
    let recent_checkpoint_date = null;
...
export class RoadmapService {
    ...
    async getWatchReport(workspace) {
    ...
    async getLastErrorBrief(workspace) {
        const lastError = await readLastError();
        if (!lastError) {
            return this.wrapClarityEnvelope({
                action: "last_error",
    ...
    async autoBootstrapIfNeeded(workspace) {
    ...
    // State Operations
    getStatePath(workspace) {
        return path.join(workspace, ".dietcode", "roadmap-state.json");
    }
    async recordMutationLineage(workspace, entry) {
        await recordMutationLineage(workspace, entry);
    }
    async getOrHydrateRuntimeState(workspace, text) {
        ...
        if (state.runtime_state) {
            if (state.runtime_state.memory)
                runtimeState.memory = state.runtime_state.memory;
            if (state.runtime_state.active_window)
                runtimeState.active_window = state.runtime_state.active_window;
            if (state.runtime_state.locks)
                runtimeState.locks = state.runtime_state.locks;
    ...
    async recordContinuationAnchor(workspace, key, value) {
    ...
    async getContinuationAnchors(workspace) {
        const runtimeState = await this.getOrHydrateRuntimeState(workspace);
        return runtimeState.memory?.continuation_anchors || {};
    }
    async acquireOrchestrationLease(workspace, agentId, taskId, durationSeconds = 300) {
        const runtimeState = await this.getOrHydrateRuntimeState(workspace);
    ...
    async releaseOrchestrationLease(workspace, agentId, taskId) {
        const runtimeState = await this.getOrHydrateRuntimeState(workspace);
    ...
    async verifyAnchorFreshness(workspace, key, expectedVersion) {
        const runtimeState = await this.getOrHydrateRuntimeState(workspace);
    ...
    async getVersionVector(workspace, key) {
        const runtimeState = await this.getOrHydrateRuntimeState(workspace);
        return runtimeState.version_vectors?.[key] || 0;
    }
    async scheduleAdmission(workspace, agentId, operation) {
        const runtimeState = await this.getOrHydrateRuntimeState(workspace);
        const now = Date.now();
    ...
    async writeState(workspace, patch) {
        const stateFile = this.getStatePath(workspace);
    ...
    async recordFileMutation(workspace, tool, filePath) {
        invalidateRoadmapWorkspaceCache(workspace);
    ...
    async recordValidation(workspace, valid, health_status, recent_checkpoint_date, phase, issue_count, bootstrap_placeholder_count) {
    ...
    // Evidence Gathering
    async gatherEvidence(workspace, roadmapText, tier) {
    ...
    assessFreshness(recentCheckpointDate, gitCommits, schemaValid, staleDays = 7, gitCommitsSinceCheckpoint, driftDetected = false) {
    ...
    // Gate State
    async buildRoadmapGateState(workspace, evidence, validation) {
        ...
        const driftDetected = cachedHash &&
            cachedHash !== "no_manifests" &&
            currentManifestHash !== "no_manifests" &&
            cachedHash !== currentManifestHash;
        const freshness = this.assessFreshness(checkpoint_date, git_commits, validation ? validation.valid : (wsState.schema_valid ?? null), getRoadmapConfig().stale_checkpoint_days, since_commits, !!driftDetected);
    ...
    buildBootstrapFillPlan(roadmapText, evidence) {
        ...
        return {
            ...
            operator_summary: tasks.length > 0
    ...
    applyBootstrapFillDraft(roadmapText, evidence) {
        ...
        return {
            ...
            preview_text: text,
            operator_summary: applied.length > 0
    ...
    async writeBootstrapAutofill(workspace, dryRun) {
        const roadmapPath = path.join(workspace, "ROADMAP.md");
        if (!(await fileExists(roadmapPath))) {
            return {
                ok: false,
    ...
    async resolveWorkspaceContext(workspace, tier = "standard", roadmapText, options) {
        ...
        if (cached) {
            return {
                workspace,
                text: String(cached.evidence._roadmap_text || ""),
                roadmapPath,
    ...
    buildOperationalPayload(action, ctx, userRequest = "") {
        ...
        if (state.runtime_state?.locks) {
            for (const lock of Object.values(state.runtime_state.locks)) {
                if (new Date(lock.expires_at).getTime() > nowMs) {
                    activeLocksCount++;
                }
            }
        }
        ...
        const tv = gateState.temporal_validity || gateState.checkpoint_freshness?.temporal_validity;
        let confidence = 1.0;
        ...
        const continuationSemantics = {
            ...
            validation_token: state.updated_at
                ...
                : "none",
            gates_to_clear: (gateState.blocking_gates || []).map((g) => g.id),
        };
        const payload = {
            action,
            success: true,
            ...
            auto_clearable_governance_only: isAutoClearableGovernanceOnly({
                ...
                blockingGates: (gateState.blocking_gates || []),
            }),
    ...
    async getOperationalStatus(workspace, contextHint = "", tier = "standard", options) {
    ...
    async checkpointBrief(workspace, context = "", userRequest = "") {
        ...
        const autofill_write = (ctx_lower.includes("apply autofill write") ||
     ...
     */
    async touchRecentCheckpointDate(workspace) {
        const roadmapPath = path.join(workspace, "ROADMAP.md");
        if (!(await fileExists(roadmapPath))) {
    ...
    async validateRoadmap(workspace) {
        const roadmapPath = path.join(workspace, "ROADMAP.md");
        let text = "";
        if (await fileExists(roadmapPath)) {
        ...
        await this.recordValidation(workspace, validation.valid, validation.health_status || null, parsed ? parsed.recent_checkpoint_date : null, phase, validation.issues.length, bootstrap_placeholder_count);
        if (validation.valid) {
        ...
        const payload = {
            ...
            recommended_next_action: next_rec,
            operator_summary: validation.valid && bootstrap_complete
            ...
            agent_next_call: validation.valid && bootstrap_complete
                ? AUTO_GOVERNANCE.continueTaskMidPass
                : validation.valid
                    ? AUTO_GOVERNANCE.continueTaskMidPass
    ...
    async getTemplateBrief(workspace) {
        ...
        const payload = {
            ...
            operator_summary: "Evidence-driven skeleton — write to ROADMAP.md; autofill and validation run at attempt_completion.",
            agent_next_call: AUTO_GOVERNANCE.continueTaskMidPass,
        };
    ...
    async applyBootstrapFillBrief(workspace, context = "") {
...
function phraseFromIssue(message) {
    const match = /[“"](.+?)[”"]/.exec(message || "");
    return match ? match[1] : message || "";
}
function lenPlaceholders(text) {
    return findBootstrapPlaceholders(text).length;
}
function buildProjectSteeringDigest(fp, fillPlan) {
    const digest = {
...
function formatAgentSteeringLine(digest) {
    const parts = [];
...
function enrichWithBootstrapFill(payload, roadmapText, evidence, bootstrap_inc) {
    if (bootstrap_inc) {
...
function suggestReplacement(phrase, fp, evidence) {
    ...
    if (operators)
        return [operators.slice(0, 240), "project_fingerprint.operators_hint"];
    if (runtime)
        return [runtime.slice(0, 240), "project_fingerprint.runtime_center_hint"];
    if (stack && brief)
        return [`Document project-specific detail for ${brief} (${stack}).`, "project_fingerprint.stack_summary"];
    if (brief)
        return [`Document project-specific detail for ${brief}.`, "project_fingerprint.steering_brief"];
    return [
        `Replace template guidance with ${archetype}-specific steering from README and repo evidence.`,
        "project_fingerprint.project_archetype",
    ];
}
function architectureHint(fp, evidence, purpose, stack) {
...
function workflowHint(fp, scripts, tests, ci) {
    const parts = [];
...
function antiGoal(archetype, brief) {
    const goals = {
    ...
    if (brief)
        return `Uncontrolled scope changes to ${brief} without a documented center of gravity.`;
    return "Uncontrolled scope sprawl without a documented center of gravity.";
}
function primaryRisk(evidence, fp) {
...
function narrativeHint(fp, _evidence, commits) {
    const brief = fp.steering_brief || fp.steering_identity || "";
    if (brief) {
...
function runtimeAuthorityHint(fp, archetype) {
    const runtime = fp.runtime_center_hint || "";
    if (runtime)
        return runtime;
...
function gitDriftHint(commits, changed) {
    if (changed.length > 0) {
...
function canonicalPathsHint(fp, soup) {
    const rec = (soup.centralization_recommendation || "").trim();
    if (rec)
        return rec.slice(0, 240);
    const scripts = fp.entry_points || [];
    if (scripts.length > 0) {
...
function suggestNowItems(evidence, limit = 3) {
    const fp = evidence.project_fingerprint || {};
    const git = evidence.git || {};
    const commits = git.recent_commits || [];
...
export function formatNowSection(items) {
...
function bootstrapSkeletonFromEvidenceAutofilled(evidence) {
    const fp = evidence.project_fingerprint || {};
    ...
    const runtime = (fp.runtime_center_hint || "Workspace project root — ROADMAP.md lives beside source, not in plugin install trees.").trim();
    ...
    const centralize = ((evidence.code_soup_audit || {}).centralization_recommendation ||
        "Document canonical paths from code_soup_pre_audit.").trim();
    const git_summary = (evidence.git || {}).recent_commits?.length > 0
...
function agentInstructions(phase, evidence) {
    const fp = evidence.project_fingerprint || {};
    const instructions = [
    ...
    if (fp.test_frameworks) {
        instructions.push(`Verification surface: ${fp.test_frameworks.slice(0, 3).join(", ")} — reference in Maintenance Gravity when relevant.`);
    }
    const uncertainty = evidence.uncertainty || [];
    if (uncertainty.length > 0) {
    ...
    if (phase === "bootstrap") {
        instructions.push("First pass: draft all 12 sections from README, architecture docs, configs, git history, and code_soup_pre_audit.");
    }
        ...
        instructions.push(`Optional preview: ${AUTO_GOVERNANCE.previewBootstrapAutofill}. Use bootstrap_fill_plan.tasks for evidence-backed replacements.`);
    }
    else {
...
function responseFormatTemplate() {
    return {
