...
export const AUTO_GOVERNANCE = {
    ...
    continueTaskMidPass: "Continue the task — roadmap governance (validate, bootstrap autofill, checkpoint date stamp) runs automatically at attempt_completion.",
    ...
    gateEvaluationFailed: "Task completion blocked: roadmap gate evaluation failed internally. Verify ROADMAP.md exists and is readable, then retry attempt_completion.",
    noManualValidate: "Do not call roadmap(action='validate') or MCP tools for governance — remediation is internal at attempt_completion.",
    previewBootstrapAutofill: "roadmap(action='apply_bootstrap_fill') — preview only; autofill writes run at attempt_completion.",
    midTaskGovernanceNote: "Governance (validate, bootstrap autofill, checkpoint date) runs automatically at attempt_completion — continue the task.",
    /** Machine-readable policy string for all roadmap payloads and preflight XML. */
    governancePolicy: "Do not call roadmap(action='validate') or MCP tools for governance — remediation is internal at attempt_completion.",
    roadmapGateRecoveryHint: "Edit ROADMAP.md to resolve governance gates — bootstrap fill, validation, and checkpoint date stamp run automatically at attempt_completion.",
    validateDiagnosticOnly: "Diagnostic only — governance runs automatically at attempt_completion; continue the task unless ROADMAP.md schema errors need repair.",
};
...
export function governanceFieldsFromStatus(status) {
    ...
    return {
        ...
        governance_mid_task: status.governance_mid_task ||
...
export function mergeGovernanceFields(payload, source) {
    return { ...payload, ...governanceFieldsFromStatus(source) };
}
/** Brief-level auto-clearable detection for steering surfaces. */
export function isAutoClearableBrief(brief) {
    if (brief.kanban_complete_allowed === true)
        return true;
    if (brief.auto_clearable_governance_only === true)
        return true;
...
export const GATE_EDIT_INSTRUCTIONS = {
    ...
    workspace_skill_installed: "Informational only — bundled auto-rolling-roadmap is a default skill when dietcode.roadmap.auto_install_skills is enabled (default: true). Not a completion gate.",
    schema_valid: "Repair ROADMAP.md — ensure all 12 required sections exist, health status is valid, and section 9 audit is present.",
    ...
    bootstrap_complete: "Replace remaining bootstrap template phrases with project-specific evidence, or let autofill run at attempt_completion.",
};
export function gateEditInstruction(gateId, fallbackFix) {
...
export function formatRemediationNote(steps) {
    if (steps.length === 0)
        return "";
    return `\n\nInternal remediation already attempted: ${steps.join("; ")}.`;
}
export function formatBlockingGatesList(gates) {
    return gates
        .map((g) => {
...
export function formatAutoRemediationSummary(steps) {
    if (steps.length === 0)
        return "";
...
export function isAutoClearableGovernanceOnly(params) {
    if (params.kanbanCompleteAllowed !== false)
        return false;
...
export function journalFollowupForMutation(bootstrapIncomplete) {
...
export function buildRoadmapGateStructuredEnvelope(input) {
    const parts = ['<roadmap_governance_recovery schema_version="1">'];
    parts.push(`<policy>${escapeXmlText(AUTO_GOVERNANCE.governancePolicy)}</policy>`);
    parts.push(`<auto_steps>${escapeXmlText([
    ...
    if (input.blockingGates && input.blockingGates.length > 0) {
        const gateBlocks = input.blockingGates
            .map((g) => {
            const edit = gateEditInstruction(g.id, g.fix);
            return (`<gate id="${escapeXmlText(g.id || "unknown")}">` +
...
//# sourceMappingURL=RoadmapAutoGovernance.js.map