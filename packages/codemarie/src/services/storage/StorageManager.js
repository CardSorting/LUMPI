...
export class StorageManager {
     ...
     */
    async cleanCacheAndTemp(maxAgeMs) {
        let freed = 0;
        const globalPath = HostProvider.get().globalStorageFsPath;
        const cacheDir = path.join(globalPath, "cache");
        const effectiveMaxAgeMs = maxAgeMs ??
            (typeof process !== "undefined" && process.env.JEST_WORKER_ID === undefined
                ? (await import("vscode")).workspace.getConfiguration("lumi").get("storage.maxCacheAgeDays", 7) *
                    24 *
                    60 *
                    60 *
                    1000
     ...
     */
    async cleanPuppeteerStorage(maxAgeMs = 3 * 24 * 60 * 60 * 1000) {
        let freed = 0;
        ...
        try {
            if (await fileExistsAtPath(puppeteerDir)) {
                ...
                for (const item of items) {
                    ...
                    if (item.startsWith(".chromium-browser-snapshots"))
                        continue;
     ...
     */
    async cleanOrphanTasksAndCheckpoints(validTaskIds) {
        let freed = 0;
        const globalPath = HostProvider.get().globalStorageFsPath;
        const tasksDir = path.join(globalPath, "tasks");
        const checkpointsDir = path.join(globalPath, "checkpoints");
     ...
     */
    async optimizeStorage(validTaskIds) {
        ...
        try {
            ...
            Logger.info(`Storage Optimization completed. Freed ${Math.round(freedBytes / 1024 / 1024)}MB. Total size: ${Math.round(breakdownAfter.totalBytes / 1024 / 1024)}MB.`);
            return {
                freedBytes,
                breakdownBefore,
                breakdownAfter,
     ...
     */
    startBackgroundMaintenance(validTaskIds) {
